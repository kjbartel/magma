...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if (cudaSuccess != cudaMalloc((void**)&dwork, (ldw*nb)*sizeof(cuFloatComplex))) {
      fprintf (stderr, "!!!! device memory allocation error (magma_chetrd_gpu)\n");
  ...
  if (upper) {
    ...
    for (i = n - nb; i >= kk; i -= nb) 
    {
      ...
      cublasGetMatrix(i+nb, nb, sizeof(cuFloatComplex), dA(0, i), ldda, A(0, i), ldwa);
    ...
    cublasSetMatrix(kk, kk, sizeof(cuFloatComplex), A(0, 0), ldwa, dA(0, 0), ldda);
  } 
  else 
  {
    ...
    cublasSetMatrix(n-i, n-i, sizeof(cuFloatComplex),
