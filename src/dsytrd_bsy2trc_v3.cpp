/*
    -- MAGMA (version 1.2.0) --
       ...
       @generated d Tue May 15 18:17:55 2012
...
extern "C"  void tile_bulge_applyQ_cpu(int WANTZ, char SIDE, int N, int NB, int Vblksiz, double *E, int LDE, double *V, double *TAU, double *T, int *INFO);
...
/*
extern "C" void TRD_hbcelr_v62sym_withQ(int N, int NB, double *A, int LDA, double *V, double *TAU, int st, int ed, int myid, int sweep, int Vblksiz);
extern "C" void TRD_type1cHLsym_withQ(int N, int NB, double *A, int LDA, double *V, double *TAU, int st, int ed, int sweep, int Vblksiz);
extern "C" void TRD_type2cHLsym_withQ(int N, int NB, double *A, int LDA, double *V, double *TAU, int st, int ed, int sweep, int Vblksiz);
extern "C" void TRD_type3cHLsym_withQ(int N, int NB, double *A, int LDA, double *V, double *TAU, int st, int ed, int sweep, int Vblksiz);
*/
/*
#define magma_dtrdtype1cbHLsym_withQ                 magma_dtrdtype1cbHLsym_withQ
#define magma_dtrdtype2cbHLsym_withQ                 magma_dtrdtype2cbHLsym_withQ
#define magma_dtrdtype3cbHLsym_withQ                magma_dtrdtype3cbHLsym_withQ
#define magma_dbulge_applyQ                           magma_dbulge_applyQ
#define magma_dstedc_withZ                        magma_dstedc_withZ
#define magma_dgemm                                 magmablas_dgemm
*/

extern "C" void magma_dtrdtype1cbHLsym_withQ(int N, int NB, double *A, int LDA, double *V, double *TAU, int st, int ed, int sweep, int Vblksiz);
extern "C" void magma_dtrdtype2cbHLsym_withQ(int N, int NB, double *A, int LDA, double *V, double *TAU, int st, int ed, int sweep, int Vblksiz);
extern "C" void magma_dtrdtype3cbHLsym_withQ(int N, int NB, double *A, int LDA, double *V, double *TAU, int st, int ed, int sweep, int Vblksiz);
extern "C" void magma_dbulge_applyQ(int WANTZ, char SIDE, int NE, int N, int NB, int Vblksiz, double *E, int LDE, double *V, double *TAU, double *T, int *INFO, double *dV, double *dT, double *dE, int copytype );
extern "C" void  magma_dstedc_withZ(char JOBZ, int N, double *D, double * E, double *Z, int LDZ);
extern "C" void  magma_dstedx_withZ(int N, int NE, double *D, double * E, double *Z, int LDZ);
...
////////////////////////////////////////////////////////////////////////////////////////////////////          
/* START CODE */
extern "C" magma_int_t magma_dsytrd_bsy2trc( int THREADS, int WANTZ, char uplo, int NE, int N, int NB, double *A1, int LDA1, double *D2, double *E2, double *dT1, int LDT1)
{
    char uplo_[2] = {uplo, 0};
...
/*
    ...
    for (j = 0; j < (N-NB); j++)
    {
        for (i = 0; i < NB+1; i++)
        {
               fprintf(trace_file,"%10d %10d %25.15e %25.15e\n",i+j+1,j+1,MAGMA_D_REAL(A1[i+j*LDA1+j]) ,  MAGMA_D_IMAG(A1[i+j*LDA1+j])  );
        }
    }        
    for (j = 0; j < NB; j++)
    {
        for (i = 0; i < NB-j; i++)
        {
               fprintf(trace_file,"%10d %10d %25.15e %25.15e\n",i+(N-NB+j)+1,N-NB+j+1,MAGMA_D_REAL(A1[i+(N-NB+j)*LDA1+(N-NB+j)]) ,  MAGMA_D_IMAG(A1[i+(N-NB+j)*LDA1+(N-NB+j)])  );
        }
    }
...
*/
    ...
    double *A2    = (double *) malloc (N*LDA2*sizeof(double));
    ...
    double *da, *NOTUSED;
    if((WANTZ>0)&(WANTZ!=4)){
       if (MAGMA_SUCCESS != magma_dmalloc( &da, N*LDA1 )) {
           //*info = MAGMA_ERR_DEVICE_ALLOC;
           return MAGMA_ERR_DEVICE_ALLOC;
       }
        
       magma_device_sync();
       magma_dsetmatrix( N, LDA1, A1, LDA1, da, LDA1 );
       if(overlapQ1==0){
           timeaplQ1 = get_time_azz();
           magma_dorgqr_2stage_gpu(N, N, N, da, LDA1, NOTUSED, dT1, NB, &INFO);
           magma_device_sync();
           //cublasGetMatrix( N, LDA1, sizeof(double), da, LDA1, A1, LDA1);
       ...
       for (j = 0; j < N ; j++) 
             for (i = 0; i < N ; i++) 
                        //fprintf(trace_file,"%10d%10d%40.30e\n",i+1,j+1,A1[j*LDA1+i]);
                         fprintf(trace_file,"%10d %10d %25.15e %25.15e\n",i+1,j+1,MAGMA_D_REAL(A1[j*LDA1+i]) ,  MAGMA_D_IMAG(A1[j*LDA1+i])  );
       fclose(trace_file);
       */
    }
    /***********************************************/
    ...
    if(CHECK)
    {
        ...
        NN=N;
        LDAINIT=NB+1;
        AINIT = (double *) malloc(MM*NN*sizeof(double) );
        memset(AINIT , 0, MM*NN*sizeof(double));        
        lapackf77_dlacpy("A", &MM, &NN, A2, &LDA2, AINIT, &LDAINIT );
    }
    /***********************************************/


    double *T     = (double *) malloc (blkcnt*LDT*Vblksiz*sizeof(double));
    double *TAU   = (double *) malloc (blkcnt*Vblksiz*sizeof(double));
    double *V     = (double *) malloc (blkcnt*LDV*Vblksiz*sizeof(double));
...
/*
       ...
       for (j = 0; j < N-1 ; j++) {
         fprintf(trace_file,"%10d %10d %25.15e %25.15e\n",j+1,j+1,MAGMA_D_REAL(A2[j*LDA2]) ,  MAGMA_D_IMAG(A2[j*LDA2])  );
         fprintf(trace_file,"%10d %10d %25.15e %25.15e\n",j+2,j+1,MAGMA_D_REAL(A2[j*LDA2+1]) ,  MAGMA_D_IMAG(A2[j*LDA2+1])  );
       }
         fprintf(trace_file,"%10d %10d %25.15e %25.15e\n",N,N,MAGMA_D_REAL(A2[(N-1)*LDA2]) ,  MAGMA_D_IMAG(A2[(N-1)*LDA2])  );
...
*/
    ...
    if(uplo==MagmaLower){
       for (i=0; i < N-1 ; i++)
       {
          D2[i] = MAGMA_D_REAL( A2[i*LDA2]);               
          E2[i] = MAGMA_D_REAL(A2[i*LDA2+1]);
       }
       D2[N-1] = MAGMA_D_REAL(A2[(N-1)*LDA2]);
    } else { /* MagmaUpper not tested yet */
        for (i=0; i<N-1; i++)
        {
            D2[i]  =  MAGMA_D_REAL(A2[i*LDA2+NB]);               
            E2[i] = MAGMA_D_REAL(A2[i*LDA2+NB-1]);
        }
        D2[N-1] = MAGMA_D_REAL(A2[(N-1)*LDA2+NB]);
    } /* end MagmaUpper */
#else
    if( uplo == MagmaLower ){
        for (i=0; i < N-1; i++) {
    ...
    if(WANTZ==0){
        ...
        magma_dstedc_withZ(MagmaNoVectors, N, D2, E2, Z, LDZ);
#if defined(USEMKL)
        mkl_set_num_threads( 1 );
#endif
        timelpk = get_time_azz()-timelpk;
    ...
    if(WANTZ>0){
...
#endif
            // call eigensolver for our resulting tridiag [D E] and form E=Q*Z
            //magma_dstedc_withZ('I', N, D2, E2, Z, LDZ);
            magma_dstedx_withZ(N, NE, D2, E2, Z, LDZ);
#if defined(USEMKL)
            mkl_set_num_threads( 1 );
#endif
            timelpk = get_time_azz()-timelpk;
       ...
       for (j = 0; j < N ; j++) 
             for (i = 0; i < N ; i++) 
                        //fprintf(trace_file,"%10d%10d%40.30e\n",i+1,j+1,A1[j*LDA1+i]);
                         fprintf(trace_file,"%10d %10d %25.15e %25.15e\n",i+1,j+1,MAGMA_D_REAL(Z[j*LDA1+i]) ,  MAGMA_D_IMAG(Z[j*LDA1+i])  );
       fclose(trace_file);
       */
        ...
        if(parallel==0){
           ...
           if(MAGMA_SUCCESS != magma_dmalloc( &dV2, dVsize )) { 
               printf ("!!!! magma_alloc failed for: dV2\n" );       
               exit(-1);                                                           
           }
    
           if(MAGMA_SUCCESS != magma_dmalloc( &dT2, blkcnt*LDT*Vblksiz )) { 
           ...
           if(WANTZ==1){
               ...
               magma_free( dT1 );
               // copy Q1 to CPU
               magma_dgetmatrix( N, LDA1, da, N, A1, LDA1 );
               // compute Q2 by applying V2 to Identity and put it into da           
               magma_dbulge_applyQ(WANTZ, 'L', NE, N, NB, Vblksiz, Q2, N, V, TAU, T, &INFO, dV2, dT2, da, 2);
               // free dT2 and allocate dZ and copy Z to dZ
               magma_device_sync();
               timeaplQ2 = get_time_azz()-timeaplQ2;
               magma_free( dT2 );
               if(MAGMA_SUCCESS != magma_dmalloc( &dZ, N*N )) { 
                ...
                Q2    = (double *) malloc (N*N*sizeof(double));
                memset(Q2 , 0, N*N*sizeof(double));       
                magma_dgetmatrix( N, LDA1, da, N, Q2, N );
                trace_file = fopen("AJETE/Q2", "w");
                for (j = 0; j < N ; j++) 
                      for (i = 0; i < N ; i++) 
                                  fprintf(trace_file,"%10d %10d %25.15e %25.15e\n",i+1,j+1,MAGMA_D_REAL(Q2[j*N+i]) ,  MAGMA_D_IMAG(Q2[j*N+i])  );
                fclose(trace_file);
                */

               timegemm = get_time_azz();
               // copy the eigenvectors to GPU
               magma_dsetmatrix( N, LDZ, Z, LDZ, dZ, N );
               // make GEMM Q2 * Z --> dV2 = da * dZ
               magma_dgemm( MagmaNoTrans, MagmaNoTrans, N, NE, N, c_one, da, N, dZ, N, c_zero, dV2, N);
               // copy Q1 to GPU --> dZ
               magma_dsetmatrix( N, LDA1, A1, LDA1, dZ, N );
               // make GEMM Q1 * (Q2 * Z) --> da = dZ * dV2
               magma_dgemm( MagmaNoTrans, MagmaNoTrans, N, NE, N, c_one, dZ, N, dV2, N, c_zero, da, N);
               magma_dgetmatrix( N, NE, da, N, A1, LDA1 );
           ...
           if(WANTZ==2){
                ...
                *==========================*/  
                   ...
                   magma_dbulge_applyQ(WANTZ, 'R', N, N, NB, Vblksiz, A1, LDA1, V, TAU, T, &INFO, dV2, dT2, da, 2);
                   magma_device_sync();
                   magma_free( dT2 );
               }
               timeaplQ2 = get_time_azz()-timeaplQ2;
                ...
                * **************************************************/
               magma_free( dT1 );
               if(MAGMA_SUCCESS != magma_dmalloc( &dZ, N*NE )) { 
                  printf ("!!!! magma_alloc failed for: dZ\n" );       
                  exit(-1);                                                           
               }
               timegemm = get_time_azz();
               // copy the eigenvectors to GPU
               magma_dsetmatrix( N, NE, Z, LDZ, dZ, N );
               //make a gemm of (Q1 * Q2) * Z = da * dZ --> dV2
               magma_dgemm( MagmaNoTrans, MagmaNoTrans, N, NE, N, c_one, da, N, dZ, N, c_zero, dV2, N);
               magma_dgetmatrix( N, NE, dV2, N, A1, LDA1 );
               timegemm = get_time_azz()-timegemm;
           }
           ...
           if(WANTZ==3){
                ...
                *==========================*/  
                   ...
                   magma_dbulge_applyQ(WANTZ, 'L', NE, N, NB, Vblksiz, Z, LDZ, V, TAU, T, &INFO, dV2, dT2, dZ, 3);
                   magma_device_sync();
               }
           ...
           if(WANTZ==5){
               ...
               timeaplQ2 = get_time_azz();
               magma_dbulge_applyQ(WANTZ, 'R', NE, N, NB, Vblksiz, A1, LDA1, V, TAU, T, &INFO, dV2, dT2, da, 2);
               magma_dgetmatrix( N, LDA1, da, N, A1, LDA1 );
               timeaplQ2 = get_time_azz()-timeaplQ2;
               timelpk = get_time_azz();
               // compute the eigenvalues using lapack routine to be able to compare to it and used as ref 
               ...
               #endif
               // call eigensolver for our resulting tridiag [D E] and form E=Q*Z
               magma_dstedc_withZ(MagmaVectors, N, D2, E2, A1, LDA1);
               #if defined(USEMKL)
               mkl_set_num_threads( 1 );
               #endif
               timelpk = get_time_azz()-timelpk;
        ...
        if(parallel==1){
           ...
           if(WANTZ==2) {
               magma_dgetmatrix( N, LDA1, da, LDA1, A1, LDA1);
           }
           // ===============================
           // relaunch thread to apply Q
           // ===============================
...
*/
    ...
    if(CHECK)
    {
       if(WANTZ!=1) JOBZ='N';
       if(WANTZ==1) JOBZ='V';
       magma_dstedc_withZ(JOBZ, N, D2, E2, Q2, LDQ2);            
       //magma_dstedc_withZ(MagmaNoVectors, N, D2, E2, Q1, LDQ1);            
    }
    ...
    if(CHECK)
    {
        ...
        * ***********************************************/
        ...
        double *ALPK    = (double *) malloc (N*LDAINIT*sizeof(double));
        memcpy(ALPK, AINIT, N*LDAINIT*sizeof(double));

        timelpk = get_time_azz();
        lapackf77_dsbtrd("N", "L", &N, &NB, ALPK, &LDAINIT, D1, E1, WORK, &N, WORK, &INFO); 
        timelpk = get_time_azz() - timelpk;
        printf("\n");                
       ...
       //magma_dstedc_withZ(JOBZ, N, D2, E2, Q1, LDQ1);
       // compare result 
       ...
       MATYPE=2;
       double NOTHING=c_zero;
       dcheck_eig_(&JOBZ, &MATYPE, &N, &NB, AINIT, &LDAINIT, &NOTHING, &NOTHING, D2 , D1, Q2, &LDQ2, WORKAJETER, RWORKAJETER, RESU );
...
static void *parallel_section(void *thread_id)
{
    ...
    double *dQ1 = core_in_all.dQ1;
    double *dT1 = core_in_all.dT1;
    int N                = core_in_all.N;
         ...
         * */ 
         ...
         if(allcores_num==1)
         {
             ...
             if(overlapQ1==1){
                 magma_device_sync();
                 timeaplQ1 = get_time_azz();
                 magma_dorgqr_2stage_gpu(N, N, N, dQ1, N, NOTUSED, dT1, NB, &INFO);
                 magma_device_sync();
                 //cublasGetMatrix( N, LDA1, sizeof(double), da, LDA1, A1, LDA1);
                 timeaplQ1 = get_time_azz()-timeaplQ1;
                 printf("  Finish applyQ1 timing= %lf \n" ,timeaplQ1); 
...
static void *applyQ_parallel_section(void *thread_id)
{
    ...
    double *dZ  = core_in_all.dE;
    double *dT2 = core_in_all.dT2;
    double *dV2 = core_in_all.dV2;
    double *Z   = core_in_all.E;
    double *T2  = core_in_all.T;
    double *V2  = core_in_all.V;
    double *TAU2= core_in_all.TAU;
...
/*################################################
    ...
    if((WANTZ==2))
    {
         ...
         if(allcores_num==1)
         {
             ...
             if(usemulticpu==1){
                 ...
                 magma_dbulge_applyQ(WANTZ, 'R', NE, N, NB, Vblksiz, NOTUSED, N, V2, TAU2, T2, &INFO, dV2, dT2, dZ, 2);
                 magma_device_sync();
          ...
          ************************************************/   
            ...
            if(my_newcore_id==lastcoreid)
            {
                ...
                //=============================================
                 timeQgpu = get_time_azz();
                 magma_dbulge_applyQ(WANTZ, 'R', N_GPU, N, NB, Vblksiz, NOTUSED, N, V2, TAU2, T2, &INFO, dV2, dT2, dZ, 2);
                 magma_device_sync();
                 timeQgpu = get_time_azz()-timeQgpu;
                 printf("  Finish Q2_GPU GGG timing= %lf \n" ,timeQgpu);
            /* I am one of the remaining cores*/
    ...
    if((WANTZ==3))
    {
         ...
         if(allcores_num==1)
         {
             ...
             if(usemulticpu==1){
                 timeQgpu = get_time_azz();
                 magma_dbulge_applyQ(WANTZ, 'L', NE, N, NB, Vblksiz, Z, LDZ, V2, TAU2, T2, &INFO, dV2, dT2, dZ, 3);
                 magma_device_sync();
                 timeQgpu = get_time_azz()-timeQgpu;
                 printf("  Finish Q2_GPU GGG timing= %lf \n" ,timeQgpu);
             }
          ...
          ************************************************/   
            ...
            if(my_newcore_id==lastcoreid)
            {
                ...
                //=============================================
                 timeQgpu = get_time_azz();
                 magma_dbulge_applyQ(WANTZ, 'L', N_GPU, N, NB, Vblksiz, Z, LDZ, V2, TAU2, T2, &INFO, dV2, dT2, dZ, 3);
                 magma_device_sync();
                 timeQgpu = get_time_azz()-timeQgpu;
                 printf("  Finish Q2_GPU GGG timing= %lf \n" ,timeQgpu);
            /* I am one of the remaining cores*/
...
static void tile_bulge_parallel(int my_core_id)
{
    ...
    double *A = core_in_all.A;
    double *V = core_in_all.V;
    double *TAU = core_in_all.TAU;
    int N = core_in_all.N;
    int NB = core_in_all.NB;
                            ...
                            if( (prog[myid+shift-1]== (sweepid-1)) )
                                {
                                    ...
                                    if(BAND==0)
                                            ...
                                            magma_dtrdtype1cbHLsym_withQ(N, NB, A, LDA, V, TAU, stind, edind, sweepid, Vblksiz);/*
                                    else if(BAND==62)
                                ...
                                if( (prog[myid-1]==sweepid) && (prog[myid+shift-1]== (sweepid-1)) )
                                    ...
                                    if(BAND==0){
                                        ...
                                        if(myid%2 == 0)
                                            magma_dtrdtype2cbHLsym_withQ(N, NB, A, LDA, V, TAU, stind, edind, sweepid, Vblksiz);
                                        else
                                            magma_dtrdtype3cbHLsym_withQ(N, NB, A, LDA, V, TAU, stind, edind, sweepid, Vblksiz);/*
                                    }else if(BAND==62){
                                            TRD_hbcelr_v62sym_withQ(N, NB, A, LDA, V, TAU, stind, edind, myid, sweepid, Vblksiz);
                                    }*/
                                    if(LOG) {
...
//       barrier(my_core_id, cores_num);
//       if(my_core_id == 0)magma_dtrdtype1cbHLsym_withQ(N, NB, A, LDA, V, TAU, N, N, N-1, Vblksiz);
        ...
        for (bg = nbGblk; bg>0; bg--)
        {
           ...
           for (m = rownbm; m>0; m--)
           {
               ...
               if(my_core_id==(myid%cores_num)){
                  if((vlen>0)&&(vnb>0))
                      lapackf77_dlarft( "F", "C", &vlen, &vnb, V(vpos), &LDV, TAU(taupos), T(tpos), &LDT);
               }
           }
        }
}
            ...
            if(SIDE=='L'){
                for (bg = nbGblk; bg>0; bg--)
                {
                   ...
                   for (m = rownbm; m>0; m--)
                   {
                       ...
                       if((vlen>0)&&(vnb>0)){
                           //lapackf77_dorgqr( "L", "N", &vlen, &corelen, &vnb, V(vpos), &LDV, TAU(taupos), E(fst,corest), &LDE,  WORK, &LWORK, &INFO );
                           //DORMQR_BLG( "L", "N", &vlen, &corelen, &vnb, &NB, V(vpos), &LDV, TAU(taupos), E(fst,corest), &LDE,  WORK, &LWORK, &INFO );
                           if(WANTZ==1){ 
                               st  = max(colst,corest);           
                               len = corelen - (st-corest);    
                               lapackf77_dlarfb( "L", "N", "F", "C", &vlen, &len, &vnb, V(vpos), &LDV, T(tpos), &LDT, E(fst,st), &LDE,  WORK, &len); 
                           }else{
                               lapackf77_dlarfb( "L", "N", "F", "C", &vlen, &corelen, &vnb, V(vpos), &LDV, T(tpos), &LDT, E(fst,corest), &LDE,  WORK, &corelen); // corelen for WORK was colpercore before 
                           }
                       }
                       if(INFO!=0) 
            ...
            if(versionR==91){		    
                for (bg =1; bg<=nbGblk; bg++)
                {
                   ...
                   for (m = 1; m<=rownbm; m++)
                   {
                       ...
                       if((vlen>0)&&(vnb>0))
                           lapackf77_dlarfb( "R", "N", "F", "C", &corelen, &vlen, &vnb, V(vpos), &LDV, T(tpos), &LDT, E(corest,fst), &LDE,  WORK, &corelen);       
                   }
                }
                }else if(versionR==92){
                ...
                for (n = nbgr; n>0; n--)
                {
                    ...
                    if((vlen>0)&&(vnb>0)){
                        for (chunkid = 0; chunkid<nbchunk; chunkid++)
                        {
                            ...
                            if(my_core_id==coreid)
                            {                 
                                lapackf77_dlarfb( "L", "N", "F", "C", &vlen, &corelen, &vnb, V(vpos), &LDV, T(tpos), &LDT, E(fst,corest), &LDE,  WORK, &corelen);    
                            } // END my_core_id=coreid
                        } // END loop over the chunk
                    } // end if vlen and vnb
                } // end for n = nbgr
