/*
    -- MAGMA (version 1.4.0-beta2) --
       ...
       @generated s Fri Jun 28 19:32:41 2013
...
/*
    float *dworkmgpu[MagmaMaxGPUs], *dWmgpu[MagmaMaxGPUs];
    for( magma_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_setdevice( dev );
        if (MAGMA_SUCCESS != magma_smalloc( &dworkmgpu[dev], nb*ldda )) {
            *info = MAGMA_ERR_DEVICE_ALLOC;
            return *info;
        }
        if (MAGMA_SUCCESS != magma_smalloc( &dWmgpu[dev], nb*ldda )) {
            *info = MAGMA_ERR_DEVICE_ALLOC;
            return *info;
        }
    }
...
//    float *datest[MagmaMaxGPUs];
    float *dworktest[MagmaMaxGPUs], *dworktestbis[MagmaMaxGPUs];
    float *dvtest[MagmaMaxGPUs], *dwtest[MagmaMaxGPUs];
...
//    float *dttest[MagmaMaxGPUs];
//    float *Atest = (float *) malloc(n*lda*sizeof(float));
//    float *Vtest = (float *) malloc(n*nb*sizeof(float));
//    float *Wtest = (float *) malloc(n*nb*sizeof(float));
    float *worktest = (float *) malloc(n*nb*sizeof(float));
    ...
    for( magma_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_setdevice( dev );
        //magma_smalloc( &datest[dev], mlocal*ldda );
        //magma_smalloc( &dttest[dev], mlocal*ldda );
        magma_smalloc( &dvtest[dev], 2*nb*lddv );
        magma_smalloc( &dwtest[dev], nb*lddw );
        magma_smalloc( &dworktest[dev], nb*ldda );
        magma_smalloc( &dworktestbis[dev], dworksiz );
        //workngpu[dev] = (float *) malloc(worksiz*sizeof(float));
    ...
    //workngpu[ngpu] = (float *) malloc(worksiz*sizeof(float));    
    // ======================
    ...
    if (upper) {
        ...
        for (i = 1; i <= n-nb; i += nb) 
        {
             ...
             // because SSYR2K use streaming and may happen 
