...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if (cudaSuccess != cudaMalloc((void**)&dwork, (ldw*nb)*sizeof(double))) {
      fprintf (stderr, "!!!! device memory allocation error (magma_dsytrd_gpu)\n");
  ...
  if (upper) {
    ...
    for (i = n - nb; i >= kk; i -= nb) 
    {
      ...
      cublasGetMatrix(i+nb, nb, sizeof(double), dA(0, i), ldda, A(0, i), ldwa);
      ...
      cublasSetMatrix(i + nb, nb, sizeof(double),
                      work, ldw, dwork, lddw);
      
      cublasDsyr2k(uplo, MagmaNoTrans, i, nb, z_neg_one, 
                   dA(0, i), ldda, dwork, 
      ...
      for (j = i; j < i+nb; ++j) {
        ...
        d[j] = MAGMA_D_GET_X( *A(j, j) );
      }
      
    }
    
    cublasGetMatrix(kk, kk, sizeof(double), dA(0, 0), ldda,
                    A(0, 0), ldwa);
    
    /*  Use CPU code to reduce the last or only block */
    lapackf77_dsytrd(uplo_, &kk, A(0, 0), &ldwa, d, e, tau, work, &lwork, &iinfo);
    
    cublasSetMatrix(kk, kk, sizeof(double), A(0, 0), ldwa, dA(0, 0), ldda);
  } 
  else 
  {
    ...
    for (i = 0; i < n-nx; i += nb) 
    {
      ...
      /*   Get the current panel */
      cublasGetMatrix(n-i, nb, sizeof(double),
                        dA(i, i), ldda,
                        A(i, i), ldwa);
      ...
      cublasSetMatrix(n-i, nb, sizeof(double),
                      work, ldw,
                      dwork, lddw);
      
      cublasDsyr2k('L', 'N', n-i-nb, nb, z_neg_one, 
                   dA(i+nb, i), ldda, 
                   &dwork[nb], lddw, d_one, 
                   dA(i+nb, i+nb), ldda);
    ...
    cublasGetMatrix(n-i, n-i, sizeof(double),
    ...
    cublasSetMatrix(n-i, n-i, sizeof(double),
