...
magma_sgetrf2_mgpu(magma_context *cntxt, magma_int_t num_gpus,
                   ...
                   float *d_lAP[MagmaMaxGPUs], float *a, magma_int_t lda,
    ...
    for( i=0; i<s; i++ ) {
        ...
        for( dd=0; dd<num_gpus; dd++ ) {
            ...
            if( d == (i+1)%num_gpus) {
                ...
                magmablas_stranspose(panel_local[d], ldpan[d], &d_lAP[d][(i%(2+num_gpus))*nb*maxm], cols, cols, nb);
            ...
            if( d == (i+1)%num_gpus )
            {
                ...
                if( nb0 > 0 ) {
                    ...
                    if( i+1 < s )
                        magmablas_stranspose(  &d_lAP[d][((i+1)%(2+num_gpus))*nb*maxm], ldda, inAT(d,loff,i_local), lddat, nb0, ldda );
                    else
                        magmablas_stranspose2( &d_lAP[d][((i+1)%(2+num_gpus))*nb*maxm], ldda, inAT(d,loff,i_local), lddat, nb0, ldda );
    ...
    if( nb0 > 0 ) {
        ...
        for( d=0; d<num_gpus; d++ ) {
            ...
            if( d == id ) {
                ...
                magmablas_stranspose2( panel_local[d], lddat, &d_lAP[d][(s%(2+num_gpus))*nb*maxm], cols, rows, nb0);
                ...
                magmablas_stranspose2( panel_local[d], nb, &d_lAP[d][(s%(2+num_gpus))*nb*maxm], cols, rows, nb0);
                magma_strsm( MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit,
                             nb1, nb0, c_one,
                             panel_local[d],     nb,
