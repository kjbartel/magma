...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if (CUBLAS_STATUS_SUCCESS != cublasAlloc((NB+nb)*ldda, sizeof(double), (void**)&dt)) {
          *info = -6;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        //                A(J, J), lda, dAup(0,J), NB);
            ...
            for (jj=0; jj<JB; jj+=nb) {
              ...
              cublasDsyrk(MagmaUpper, MagmaTrans, jb, jj, 
                          mdone, dAup(0,  j), NB, 
                          done,  dAup(jj, j), NB);

                  /* send the diagonal-block to CPU */
              cudaMemcpy2DAsync(  A  (J, j), lda*sizeof(double), 
                                 dAup(0, j), NB *sizeof(double), 
                                 sizeof(double)*(jj+jb), jb,
                                 cudaMemcpyDeviceToHost, stream[1]);
              //cudaMemcpy2DAsync(  A  ( j, j), lda*sizeof(double), 
              //                   dAup(jj, j), NB *sizeof(double), 
              //                   sizeof(double)*jb, jb,
              //                   cudaMemcpyDeviceToHost, stream[1]);
              ...
              if ( (j+jb) < n ) {
                    ...
                    cudaMemcpy2DAsync(dAup(jj, j), NB  * sizeof(double), 
          ...
          for( J=0; J<n; J+=NB ) {
            ...
            for (jj=0; jj<JB; jj+=nb) {
              ...
              cublasSetMatrix((n-J), nb, sizeof(double), 
                              A(J, j), lda, dT(J, 0), ldda);

                  /* update the current big-panel    *
                   * using the previous block-column */
              cublasDsyrk(MagmaLower, MagmaNoTrans, JB, nb,
                          mdone, dT(J, 0), ldda, 
                          done,  dA(J, 0), ldda);
                  if( J+JB < n )
            ...
            for (jj=0; jj<JB; jj+=nb) {
              ...
              /* Update the current diagonal block */
              cublasDsyrk(MagmaLower, MagmaNoTrans, jb, jj,
                          mdone, dA(j, 0), ldda, 
                          done,  dA(j, jj), ldda);
              ...
              if ( (j+jb) < n) {
                    /* send the diagonal-block to GPU */
                cudaMemcpy2DAsync( dA(j, jj), ldda*sizeof(double), 
                                   A(j,   j), lda *sizeof(double), 
                                   sizeof(double)*jb, jb, 
                                   cudaMemcpyHostToDevice,stream[0]);
