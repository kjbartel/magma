...
/* ============ definitions required for cgetrf_mc ====================== */
#include <quark.h>
...
magma_cgetrf_mc(magma_context *cntxt,
                int *m, int *n,
...
extern "C" magma_int_t
magma_cgetrf2_mgpu(magma_context *cntxt, magma_int_t num_gpus,
                   magma_int_t m, magma_int_t n, magma_int_t nb, magma_int_t offset,
                   magmaFloatComplex **d_lAT, magma_int_t lddat, magma_int_t *ipiv,
                   magmaFloatComplex *d_lAP[MagmaMaxGPUs], magmaFloatComplex *a, magma_int_t lda,
...
#define W(j) (w+((j)%num_gpus)*nb*ldw)
    ...
    *info = 0;
    ...
    if( nb0 == nb )
        magmablas_ctranspose(  d_lAP[0], maxm, inAT(0,0,0), lddat, nb0, maxm );
    else
        magmablas_ctranspose2( d_lAP[0], maxm, inAT(0,0,0), lddat, nb0, maxm );
    ...
    for( i=0; i<s; i++ ) {
...
#ifdef PANEL_FACT_MC
        cntxt->nb = 12;
        magma_cgetrf_mc(cntxt, &rows, &nb, W(i), &ldw, ipiv+i*nb, &iinfo);
#else
        ...
        for( dd=0; dd<num_gpus; dd++ ) {
            ...
            if( d == (i+1)%num_gpus) {
                ...
                magmablas_ctranspose(panel_local[d], ldpan[d], &d_lAP[d][(i%(2+num_gpus))*nb*maxm], cols, cols, nb);
            ...
            if( d == (i+1)%num_gpus )
            {
                ...
                if( nb0 > 0 ) {
                    ...
                    if( i+1 < s )
                        magmablas_ctranspose(  &d_lAP[d][((i+1)%(2+num_gpus))*nb*maxm], ldda, inAT(d,loff,i_local), lddat, nb0, ldda );
                    else
                        magmablas_ctranspose2( &d_lAP[d][((i+1)%(2+num_gpus))*nb*maxm], ldda, inAT(d,loff,i_local), lddat, nb0, ldda );
    ...
    if( nb0 > 0 ) {
        ...
        for( d=0; d<num_gpus; d++ ) {
            ...
            if( d == id ) {
                ...
                magmablas_ctranspose2( panel_local[d], lddat, &d_lAP[d][(s%(2+num_gpus))*nb*maxm], cols, rows, nb0);
                ...
                magmablas_ctranspose2( panel_local[d], nb, &d_lAP[d][(s%(2+num_gpus))*nb*maxm], cols, rows, nb0);
                magma_ctrsm( MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit,
                             nb1, nb0, c_one,
                             panel_local[d],     nb,
