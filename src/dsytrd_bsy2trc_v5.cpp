...
extern "C" {
    ...
    void magma_dtrdtype1cbHLsym_withQ_v2(magma_int_t n, magma_int_t nb, double *A, magma_int_t lda, double *V, magma_int_t ldv, double *TAU,
                                         magma_int_t st, magma_int_t ed, magma_int_t sweep, magma_int_t Vblksiz, double *work);
    void magma_dtrdtype2cbHLsym_withQ_v2(magma_int_t n, magma_int_t nb, double *A, magma_int_t lda, double *V, magma_int_t ldv, double *TAU,
                                         magma_int_t st, magma_int_t ed, magma_int_t sweep, magma_int_t Vblksiz, double *work);
    void magma_dtrdtype3cbHLsym_withQ_v2(magma_int_t n, magma_int_t nb, double *A, magma_int_t LDA, double *V, magma_int_t ldv, double *TAU,
                                         magma_int_t st, magma_int_t ed, magma_int_t sweep, magma_int_t Vblksiz, double *work);


    void magma_dbulge_applyQ_v2(char side, magma_int_t NE, magma_int_t N, magma_int_t NB, magma_int_t Vblksiz,
                                double *dE, magma_int_t ldde, double *V, magma_int_t ldv, double *T, magma_int_t ldt, magma_int_t *info);

    void magma_dstedc_withZ(char JOBZ, magma_int_t n, double *D, double * E, double *Z, magma_int_t LDZ);
    void magma_dstedx_withZ(magma_int_t n, magma_int_t ne, double *D, double * E, double *Z, magma_int_t LDZ);

    magma_int_t magma_dorgqr_2stage_gpu(magma_int_t m, magma_int_t n, magma_int_t k, double *da, magma_int_t ldda,
...
static void tile_bulge_parallel(magma_int_t my_core_id, magma_int_t cores_num, double *A, magma_int_t lda,
                                double *V, magma_int_t ldv, double *TAU, magma_int_t n, magma_int_t nb, magma_int_t nb_tiles,
                                magma_int_t band, magma_int_t grsiz, magma_int_t Vblksiz, volatile magma_int_t *prog);

static void tile_bulge_computeT_parallel(magma_int_t my_core_id, magma_int_t cores_num, double *V, magma_int_t ldv, double *TAU,
...
static void tile_bulge_applyQ(magma_int_t wantz, char side, magma_int_t n_cpu, magma_int_t n, magma_int_t nb, magma_int_t Vblksiz,
                              double *E, magma_int_t lde, double *V, magma_int_t ldv, double *TAU, double *T, magma_int_t ldt);
...
public:
    ...
    bulge_data(magma_int_t threads_num_, magma_int_t n_, magma_int_t nb_, magma_int_t nbtiles_,
               ...
               double *T_, magma_int_t ldt_, magma_int_t computeQ1_, double *dQ1_, magma_int_t lddq1_,
               double *dT1_, volatile magma_int_t* prog_)
    :
...
public:
    ...
    applyQ_data(magma_int_t threads_num_, magma_int_t n_, magma_int_t ne_, magma_int_t n_gpu_,
                ...
                double *E_, magma_int_t lde_, double *V_, magma_int_t ldv_, double *TAU_,
...
extern "C" magma_int_t magma_dsytrd_bsy2trc_v5(magma_int_t threads, magma_int_t wantz, char uplo, magma_int_t ne, magma_int_t n, magma_int_t nb,
                                               double *A, magma_int_t lda, double *D, double *E,
                                               /*double* dQ1, magma_int_t lddq1,*/ double *dT1, magma_int_t ldt1)
{
    ////////////////////////////////////////////////////////////////
    double *da, *Z;
    if((wantz>0)/*&&(wantz!=4)*/){
    ...
    if(wantz>0)
       magma_dsetmatrix( n, n, A, lda, dQ1, lddq1 );
    ...
    magma_dmalloc_cpu(&T,   blkcnt*ldt*Vblksiz);
    magma_dmalloc_cpu(&TAU, blkcnt*Vblksiz    );
    magma_dmalloc_cpu(&V,   blkcnt*ldv*Vblksiz);
    ...
    if(uplo==MagmaLower){
        ...
        {
            D[i] = MAGMA_D_REAL(A2[i*lda2  ]);
            E[i] = MAGMA_D_REAL(A2[i*lda2+1]);
        }
        D[n-1] = MAGMA_D_REAL(A2[(n-1)*lda2]);
    } else { /* MagmaUpper not tested yet */
        for (magma_int_t i=0; i<n-1; i++)
        {
            D[i]  =  MAGMA_D_REAL(A2[i*lda2+nb]);
            E[i] = MAGMA_D_REAL(A2[i*lda2+nb-1]);
        }
        D[n-1] = MAGMA_D_REAL(A2[(n-1)*lda2+nb]);
    } /* end MagmaUpper */
#else
    if( uplo == MagmaLower ){
        for (magma_int_t i=0; i < n-1; i++) {
    ...
    if(wantz==0){
        ...
        magma_dstedc_withZ(MagmaNoVectors, n, D, E, NULL, 1);
        //magma_dstedc_withZ(MagmaNoVectors, n, D, E, Z, ldz);
        ...
        magma_dmalloc_cpu (&Z, n*n);

        magma_dstedx_withZ(n, ne, D, E, Z, ldz);
    ...
    omp_set_num_threads(1);
        ...
        if(wantz==2){
            ...
            if(n_gpu < n)
            {
                ...
                magma_dgetmatrix( n-n_gpu, n, &da[n_gpu], lda, &A[n_gpu], lda);
                ...
                magma_dsetmatrix( n-n_gpu, n, &A[n_gpu], lda, &da[n_gpu], lda);

                /*============================
                 *  use only GPU
                 *==========================*/
            }else{
                magma_dbulge_applyQ_v2('R', n, n, nb, Vblksiz, dQ1, n, V, ldv, T, ldt, &info);
            }
            ...
            double *dZ2;
            if(MAGMA_SUCCESS != magma_dmalloc( &dZ2, n*ne )) {
                printf ("!!!! magma_alloc failed for: dZ2\n" );
                exit(-1);
            }
            magma_dgemm( MagmaNoTrans, MagmaNoTrans, n, ne, n, c_one, da, n, dZ, n, c_zero, dZ2, n);
            magma_dgetmatrix( n, ne, dZ2, n, A, lda );
            magma_free(dZ2);
            ...
            if(n_gpu < ne)
            {
                ...
                magma_dsetmatrix(n, ne-n_gpu, Z + n_gpu*ldz, ldz, dZ + n_gpu*ldz, n);
                ...
                magma_dsetmatrix(n, ne, Z, ldz, dZ, n);
                magma_dbulge_applyQ_v2('L', ne, n, nb, Vblksiz, dZ, n, V, ldv, T, ldt, &info);
                magma_device_sync();
            }
            timeaplQ2 = magma_wtime()-timeaplQ2;
            ...
            if (wantz==3){
                ...
                magma_dgemm( MagmaNoTrans, MagmaNoTrans, n, ne, n, c_one, dQ1, lddq1, dZ, n, c_zero, dZ2, n);
                magma_dgetmatrix( n, ne, dZ2, n, A, lda );
                magma_free(dZ2);
                timegemm = magma_wtime()-timegemm;
            }else{
                 ...
                 * apply Q1 to (Q2*Z)
                 * **************************************************/
                printf("calling dormqr\n");
                timegemm = magma_wtime();
                magma_dormqr_gpu_2stages(MagmaLeft, MagmaNoTrans, n-nb, ne, n-nb, dQ1+nb, lddq1,
                                         dZ+nb, n, dT1, nb, &info);
                magma_free(dT1);
                magma_dgetmatrix( n, ne, dZ, n, A, lda );
                timegemm = magma_wtime()-timegemm;
            }
    ...
    if((wantz>0))
    {
        ...
        if(allcores_num==1)
        {
            ...
            if(computeQ1==1){
                ...
                magma_dorgqr_2stage_gpu(n, n, n, dQ1, lddq1, NULL, dT1, nb, &info);
                magma_device_sync();
...
static void *applyQ_parallel_section(void *arg)
{
    ...
    if((wantz==2))
    {
        ...
        if(my_core_id == 0)
        {
            ...
            magma_dbulge_applyQ_v2('R', n, n, nb, Vblksiz, dE, ldde, V, ldv, T, ldt, &info);
            magma_device_sync();
            ...
            double* E_loc = E + n_gpu+ n_loc * (my_core_id-1);
...
static void tile_bulge_applyQ(magma_int_t wantz, char side, magma_int_t n_loc, magma_int_t n, magma_int_t nb, magma_int_t Vblksiz,
                              double *E, magma_int_t lde, double *V, magma_int_t ldv, double *TAU, double *T, magma_int_t ldt)//, magma_int_t* info)
{
    //%===========================
    //%   local variables
    //%===========================
