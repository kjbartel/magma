...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if ((m == n) && (m % 32 == 0) && (lda%32 == 0))
      magmablas_cinplace_transpose( dAT, lda, ldat );
    else {
      if ( CUBLAS_STATUS_SUCCESS != 
           cublasAlloc(maxm*maxn, sizeof(cuFloatComplex), (void**)&dAT) )
        return MAGMA_ERR_CUBLASALLOC;
      
      magmablas_ctranspose2( dAT, ldat, dA, lda, m, n );
    }
    
    magma_cgeqrf2_gpu(n, m, dAT, lda, tau, &iinfo);

    if ((m == n) && (m % 32 == 0) && (lda%32 == 0))
      magmablas_cinplace_transpose( dAT, ldat, lda );
    else {
