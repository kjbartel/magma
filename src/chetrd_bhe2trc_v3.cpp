/*
    -- MAGMA (version 1.3.0) --
       ...
       @generated c Wed Nov 14 22:53:27 2012
...
extern "C" void cmp_vals(magma_int_t n, float *wr1, float *wr2, float *nrmI, float *nrm1, float *nrm2);
extern "C" void ccheck_eig_(char *JOBZ, magma_int_t  *MATYPE, magma_int_t  *N, magma_int_t  *NB,
                       cuFloatComplex* A, magma_int_t  *LDA, float *AD, float *AE, float *D1, float *EIG,
                    cuFloatComplex *Z, magma_int_t  *LDZ, cuFloatComplex *WORK, float *RWORK, float *RESU);
#endif


extern "C"  magma_int_t plasma_ceildiv(magma_int_t a, magma_int_t b);
extern "C"  void tile_bulge_applyQ_cpu(magma_int_t WANTZ, char SIDE, magma_int_t N, magma_int_t NB, magma_int_t Vblksiz, cuFloatComplex *E, magma_int_t LDE, cuFloatComplex *V, cuFloatComplex *TAU, cuFloatComplex *T, magma_int_t *INFO);
...
////////////////////////////////////////////////////////////////////////////////////////////////////          
/* START CODE */
extern "C" magma_int_t magma_chetrd_bhe2trc( magma_int_t THREADS, magma_int_t WANTZ, char uplo, magma_int_t NE, magma_int_t N, magma_int_t NB, cuFloatComplex *A1, magma_int_t LDA1, float *D2, float *E2, cuFloatComplex *dT1, magma_int_t LDT1)
{
    char uplo_[2] = {uplo, 0};
...
#if (defined(PRECISION_s) || defined(PRECISION_d))
                           N_GPU = (int) (0.68*(float)NE);
#else
                           N_GPU = (int) (0.72*(float)NE);
#endif
                           N_GPU = (N_GPU/64)*64;
                           N_CPU = NE-N_GPU;
                   }else if(THREADS>5){
#if (defined(PRECISION_s) || defined(PRECISION_d))
                           N_GPU = (int) (0.84*(float)NE);
#else
                           N_GPU = (int) (0.86*(float)NE);
#endif
                           N_GPU = (N_GPU/64)*64;
                           N_CPU = NE-N_GPU;
                   }else{
...
*/
    ...
    if(CHECK)
    {
        ...
        * ***********************************************/
        ...
        timelpk = magma_wtime() - timelpk;
        printf("\n");                
        printf("  Time CHBTRD-MKL-LAPACK                      :   %f    N : %10d    NB : %10d \n\n\n",timelpk, N, NB );
        /* call eigensolver */
                ...
                for (bg = nbGblk; bg>0; bg--)
                {
                   ...
                   for (m = rownbm; m>0; m--)
                   {
                       ...
                       if((vlen>0)&&(vnb>0)){
                           ...
                           //CUNMQR_BLG( "L", "N", &vlen, &corelen, &vnb, &NB, V(vpos), &LDV, TAU(taupos), E(fst,corest), &LDE,  WORK, &LWORK, &INFO );
                           if(WANTZ==1){ 
                               st  = max(colst,corest);           
