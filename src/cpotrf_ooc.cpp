...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if (CUBLAS_STATUS_SUCCESS != cublasAlloc((NB+nb)*ldda, sizeof(cuFloatComplex), (void**)&dt)) {
          *info = -6;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        //                A(J, J), lda, dAup(0,J), NB);
              ...
              cublasSetMatrix(nb, (n-J), sizeof(cuFloatComplex), 
                              A(j, J), lda, dTup(0, J), nb);

                  /* update the current big-panel *
                   * using the previous block-row */
              cublasCherk(MagmaUpper, MagmaConjTrans, JB, nb,
                          mdone, dTup(0, J), nb, 
                          done,  dAup(0, J), NB);
                  if( (J+JB) < n ) 
            ...
            for (jj=0; jj<JB; jj+=nb) {
              ...
              cublasCherk(MagmaUpper, MagmaConjTrans, jb, jj, 
                          mdone, dAup(0,  j), NB, 
                          done,  dAup(jj, j), NB);

                  /* send the diagonal-block to CPU */
              cudaMemcpy2DAsync(  A  (J, j), lda*sizeof(cuFloatComplex), 
                                 dAup(0, j), NB *sizeof(cuFloatComplex), 
                                 sizeof(cuFloatComplex)*(jj+jb), jb,
                                 cudaMemcpyDeviceToHost, stream[1]);
              //cudaMemcpy2DAsync(  A  ( j, j), lda*sizeof(cuFloatComplex), 
              //                   dAup(jj, j), NB *sizeof(cuFloatComplex), 
              ...
              if ( (j+jb) < n ) {
                    ...
                    cudaMemcpy2DAsync(dAup(jj, j), NB  * sizeof(cuFloatComplex), 
            ...
            //cudaMemcpy2DAsync(  A  (j,  j+jb), lda*sizeof(cuFloatComplex),
                    //                   dAup(jj, j+jb), NB *sizeof(cuFloatComplex),
                ...
                if( J+JB < n )
            cudaMemcpy2DAsync(  A  (J, J+JB), lda*sizeof(cuFloatComplex),
                               dAup(0, J+JB), NB *sizeof(cuFloatComplex),
                               sizeof(cuFloatComplex)*JB, n-J-JB, 
                               cudaMemcpyDeviceToHost,stream[2]);
          }
        } else {
          ...
          for( J=0; J<n; J+=NB ) {
            ...
            for (jj=0; jj<JB; jj+=nb) {
              ...
              cublasSetMatrix((n-J), nb, sizeof(cuFloatComplex), 
                              A(J, j), lda, dT(J, 0), ldda);

                  /* update the current big-panel    *
                   * using the previous block-column */
              cublasCherk(MagmaLower, MagmaNoTrans, JB, nb,
                          mdone, dT(J, 0), ldda, 
                          done,  dA(J, 0), ldda);
                  if( J+JB < n )
            ...
            for (jj=0; jj<JB; jj+=nb) {
              ...
              /* Update the current diagonal block */
              cublasCherk(MagmaLower, MagmaNoTrans, jb, jj,
                          mdone, dA(j, 0), ldda, 
                          done,  dA(j, jj), ldda);
              ...
              cudaMemcpy2DAsync(  A(j,j),  lda *sizeof(cuFloatComplex),
                                 dA(j,jj), ldda*sizeof(cuFloatComplex),
                                 sizeof(cuFloatComplex)*jb, jb,
                                 cudaMemcpyDeviceToHost,stream[1]);
              ...
              if ( (j+jb) < n) {
                    /* send the diagonal-block to GPU */
                cudaMemcpy2DAsync( dA(j, jj), ldda*sizeof(cuFloatComplex), 
                                   A(j,   j), lda *sizeof(cuFloatComplex), 
                                   sizeof(cuFloatComplex)*jb, jb, 
                                   cudaMemcpyHostToDevice,stream[0]);
            ...
            cudaMemcpy2DAsync(  A(J+JB, J), lda *sizeof(cuFloatComplex),
                               dA(J+JB, 0), ldda*sizeof(cuFloatComplex),
