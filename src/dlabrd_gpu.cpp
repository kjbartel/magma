/*
    -- MAGMA (version 1.1) --
       ...
       @generated d Sun Nov 13 20:48:24 2011
    ...
    if (m >= n) {
        ...
        for (i__ = 1; i__ <= nb; ++i__) {
            ...
            if (i__ < n) {
                ...
                // 2. Multiply ---------------------------------------------
                cublasDgemv(MagmaTrans, i__2, i__3, c_one, 
                            da + (i__-1) + ((i__-1) + 1) * (ldda), ldda, 
                            da + (i__-1) + (i__-1) * (ldda), c__1, c_zero, 
                            dy + i__ + 1 + i__ * y_dim1, c__1);
                
                // 3. Put the result back ----------------------------------
                cudaMemcpy2DAsync(y+i__+1+i__*y_dim1, y_dim1*sizeof(double),
                                  dy+i__+1+i__*y_dim1, y_dim1*sizeof(double),
                                  sizeof(double)*i__3, 1,
                ...
                cublasSetVector(i__3, sizeof(double),
                                ...
                                da+(i__-1)+((i__-1)+1)*(ldda), ldda);
                // 2. Multiply ---------------------------------------------
                //cublasDcopy(i__3, da+(i__-1)+((i__-1)+1)*(ldda), ldda,
                //            dy + 1 + lddy, 1);
                cublasDgemv('N', i__2, i__3, c_one,
                ...
                // 3. Put the result back ----------------------------------
                cudaMemcpy2DAsync(x+i__+1+i__*x_dim1, x_dim1*sizeof(double),
                                  dx+i__+1+i__*x_dim1, x_dim1*sizeof(double),
      ...
      for (i__ = 1; i__ <= nb; ++i__) {
        ...
        if (i__ < m) {
          ...
          // 2. Multiply ---------------------------------------------
          //cublasDcopy(i__3, da+(i__-1)+(i__-1)*(ldda), ldda,
          //            dy + 1 + lddy, 1);
          cublasDgemv(MagmaNoTrans, i__2, i__3, c_one,
