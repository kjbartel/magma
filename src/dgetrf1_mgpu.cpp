...
#else
  #define magmablas_dtrsm magma_dtrsm
#endif
...
#define inAT(id,i,j) (d_lAT[(id)] + ((offset)+(i)*nb)*lddat + (j)*nb)
    ...
    double *d_panel[4], *panel_local[4];
      ...
      for( i=0; i<s; i++ )
            {
                ...
                if ( i>0 ){
                    ...
                    magmablas_dtrsm( MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit, 
                ...
                for( d=0; d<num_gpus; d++ ) {
                  ...
                  /* apply the pivoting */
                  if( d == 0 ) 
                    magmablas_dpermute_long2( inAT(d,0,0), lddat, ipiv, nb, i*nb );
                  else
            ...
            for( d=0; d<num_gpus; d++ ) {
              ...
              if( nb0 > 0 ) {
                if( d == 0 ) 
                  magmablas_dpermute_long2( inAT(d,0,0), lddat, ipiv, nb0, s*nb );
                else
