...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if (CUBLAS_STATUS_SUCCESS != cublasAlloc((n)*ldda + nb*lddwork, sizeof(double), (void**)&da)) {
        *info = -9;
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        cudaMemcpy2DAsync(da_ref(0, 0), ldda*sizeof(double),
                          a_ref(0, 0),  lda *sizeof(double),
        ...
        for (i = k - kk + ki; i >= k -kk; i -= nb) {
            ...
            if (i< k - kk + ki){
                /* 1. Copy asynchronously the current panel to the CPU.
                   ...
                   to the CPU)                                        */
                rows = m - k + i + ib;
                cudaMemcpy2DAsync( a_ref(0, n-k+i),  lda *sizeof(double),
                                   da_ref(0, n-k+i), ldda*sizeof(double),
                                   sizeof(double)*rows, ib,
                                   cudaMemcpyDeviceToHost, stream[1]);

                cudaMemcpy2DAsync( a_ref(rows, n-k+i),  lda *sizeof(double),
                                   da_ref(rows, n-k+i), ldda*sizeof(double),
                                   sizeof(double)*(m-rows), ib,
        ...
        cublasGetMatrix(m, nu, sizeof(double),
