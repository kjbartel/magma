...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    double *da;
    cublasStatus status;
    status = cublasAlloc(N*ldda+2*N*nb+nb*nb, sizeof(double), (void**)&da);
    ...
    if (nb < nbmin || nb >= nh) {
      ...
      cublasSetMatrix(N, N-ilo+1, sizeof(double), a+(ilo-1)*(lda), lda, d_A, ldda);
      ...
      for (i__ = ilo; i__ < ihi - nb; i__ += nb) {
        ...
        cublasGetMatrix(ihi-i__+1, ib, sizeof(double), 
    ...
    if (!(nb < nbmin || nb >= nh))
        cublasGetMatrix( n, n-i__+1, sizeof(double), 
