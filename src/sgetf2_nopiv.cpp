    ...
    for (j = 1; j <= i__1; ++j) 
      {
        ...
        if (!MAGMA_S_EQUAL(a[i__2], c_zero)) {
          ...
          if (j < *m) {
            if (MAGMA_S_ABS(a[j + j * a_dim1]) >= sfmin) 
              {
                i__2 = *m - j;
                MAGMA_S_ASSIGN(z__1, MAGMA_S_DIV(c_one, a[j + j * a_dim1]));
                blasf77_sscal(&i__2, &z__1, &a[j + 1 + j * a_dim1], &c__1);
              } 
            else 
              {
                i__2 = *m - j;
                for (i__ = 1; i__ <= i__2; ++i__) {
                  i__3 = j + i__ + j * a_dim1;
                  MAGMA_S_ASSIGN(a[i__3], MAGMA_S_DIV(a[j + i__ + j * a_dim1], a[j + j*a_dim1]));
                }
              }
          }
        ...
        if (j < min(*m,*n)) {  
          ...
          sger_(&i__2, &i__3, &z__1, &a[j + 1 + j * a_dim1], &c__1,
