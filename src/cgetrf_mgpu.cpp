/*
    -- MAGMA (version 1.2.1) --
       ...
       @generated c Thu Jun 28 12:30:38 2012
...
#define inAT(id,i,j) (d_lAT[(id)] + (i)*nb*lddat + (j)*nb)
    ...
    cuFloatComplex *d_lAT[MagmaMaxGPUs], *d_lAP[MagmaMaxGPUs];
    cuFloatComplex *d_panel[MagmaMaxGPUs], *panel_local[MagmaMaxGPUs], *work;
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        magma_setdevice(i);
                ...
                /* workspaces */
            if (MAGMA_SUCCESS != magma_cmalloc( &d_lAP[i], nb*maxm )) {
                for( j=0; j<i; j++ ) {
                    magma_setdevice(j);
                    magma_free( d_lAP[j]   );
            ...
            if (MAGMA_SUCCESS != magma_cmalloc( &d_panel[i], nb*maxm )) {
                for( j=0; j<=i; j++ ) {
                    magma_setdevice(j);
                    magma_free( d_lAP[j] );
                }
          ...
          /* cpu workspace */
          lddwork = maxm;
          if (MAGMA_SUCCESS != magma_cmalloc_pinned( &work, lddwork*nb )) {
              for(i=0; i<num_gpus; i++ ) {
                  magma_setdevice(i);
                  magma_free( d_lAP[i]   );
      ...
      s = mindim / nb;
          for( i=0; i<s; i++ )
            {
                ...
                for( d=0; d<num_gpus; d++ ) {
                    ...
                    /* apply the pivoting */
                    if( d == 0 ) 
                        magmablas_cpermute_long2( lddat, d_lAT[d], lddat, ipiv, nb, i*nb );
                    else
          ...
          for( d=0; d<num_gpus; d++ ) {
              ...
              if( nb0 > 0 ) {
                  if( d == 0 ) 
                      magmablas_cpermute_long2(lddat, d_lAT[d], lddat, ipiv, nb0, s*nb);
                  else
