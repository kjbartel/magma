...
/*  -- MAGMA (version 1.1) --
    ...
    Purpose
    =======
    CGETRF3 computes an LU factorization of a general M-by-N matrix A
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= n) 
    ...
    else 
      {
          ...
          for(i=0; i<num_gpus; i++)
            {
              ...
              if ( CUBLAS_STATUS_SUCCESS != cublasAlloc((2*nb + ldat_local[i])*maxm,
                                                        sizeof(cuFloatComplex), (void**)&d_lA[i]))
                {
                  for( j=0; j<i; j++ ) {
                    cudaSetDevice(j);
                    cublasFree( d_lA[j] );
          ...
          magmablas_csetmatrix_transpose2( m, n, a, lda, d_lAT, ldat_local,
          ...
          if ( cudaSuccess != cudaMallocHost( (void**)&work, 
                                              lddwork*nb*sizeof(cuFloatComplex) ) ) 
            {
              for(i=0; i<num_gpus; i++ ) {
                cudaSetDevice(i);
                cublasFree( d_lA[i] );
          ...
          for( i=0; i<s; i++ )
            {
              ...
              for( d=0; d<num_gpus; d++ ) {
                ...
                magmablas_ctranspose2s(panel_local[d], ldpan[d], d_lAP[d], maxm, cols, nb, 
                                       &streaml[d][0]);
...
#endif
          /* save on output */
          magmablas_cgetmatrix_transpose2( m, n, d_lAT, ldat_local, a, lda,
              d_lA, maxm, nb, num_gpus, streaml );
