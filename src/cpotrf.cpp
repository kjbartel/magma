...
magma_cpotrf_m(magma_int_t num_gpus, char uplo, magma_int_t n,
    ...
    if (nb <= 1 || nb >= n) {
        ...
        if (upper) {
            ...
            for (j=0; j<n; j += nb) {
                ...
                jb = min(nb, (n-j));
                magma_csetmatrix( jb, (n-j), A(j, j), lda, dA(j, j), ldda );
                
                magma_cherk(MagmaUpper, MagmaConjTrans, jb, j, 
                            d_neg_one, dA(0, j), ldda, 
                            d_one,     dA(j, j), ldda);

                magma_cgetmatrix_async( (j+jb), jb,
                                        dA(0, j), ldda,
                                        A(0, j),  lda, stream[1] );
            ...
            for (j=0; j<n; j+=nb) {
                ...
                //  for non-positive-definiteness. Computing MIN 
                jb = min(nb, (n-j));
                magma_csetmatrix( (n-j), jb, A(j, j), lda, dA(j, j), ldda );

                magma_cherk(MagmaLower, MagmaNoTrans, jb, j,
                            d_neg_one, dA(j, 0), ldda, 
                            d_one,     dA(j, j), ldda);
                /*
                magma_cgetmatrix_async( jb, j+jb,
                                        dA(j,0), ldda,
                                        A(j, 0), lda, stream[1] );
                */
