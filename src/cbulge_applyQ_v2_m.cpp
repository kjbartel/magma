                        ...
                        magma_int_t *info)
{
    ...
    */
    ...
    magmaFloatComplex *dE[MagmaMaxGPUs], *dwork[MagmaMaxGPUs], *dwvt[MagmaMaxGPUs];
    ...
    for (dev=0; dev < ngpu; ++dev){
        ...
        if(MAGMA_SUCCESS != magma_cmalloc( &dwork[dev], 2*lddwork*Vblksiz + 2*Vchunksiz* (Vblksiz* (lddv+lddt)) + lddwvt*Vblksiz )) {
                        ...
                        if(flip==0){
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for(magma_int_t i=0; i<ie_loc; i+= sz_bl){
                                    ...
                                    //magma_clarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork[dev], lddwork);
                                    magma_clarfb_gpu_gemm( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork[dev], ib, dwvt[dev], Vm);
                                }
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for(magma_int_t i=0; i<ie_loc; i+= sz_bl){
                                    ib = min(sz_bl, ie_loc-i);
                                    //magma_clarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork[dev], ib);
                                    magma_clarfb_gpu_gemm( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork[dev], ib, dwvt[dev], Vm);
                                }
