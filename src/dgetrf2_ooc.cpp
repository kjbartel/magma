...
magma_dgetrf_ooc(magma_int_t m, magma_int_t n, double *a, magma_int_t lda, 
...
/*  -- MAGMA (version 1.1) --
    ...
    Purpose
    =======
    DGETRF_OOC computes an LU factorization of a general M-by-N matrix A
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        for( I=0; I<n; I+=NB ) {
                  ...
                  magmablas_dsetmatrix_transpose( M, N, A(0,I), lda, dAT, maxn, dA, maxm, nb);
                  ...
                  for( offset = 0; offset<min(m,I); offset+=NB ) 
                    {
                      ...
                      NBk    = min( m-offset, NB );
                      magmablas_dpermute_long3( dAT, maxn, ipiv, NBk, offset );
                      ...
                      for( jj=0, ib=offset/nb; jj<NBk; jj+=nb, ib++ ) 
                        {
                          ...
                          cublasSetMatrix( M-ii, nb, sizeof(double), A(ii, ii), lda, dA, rows);
                          magmablas_dtranspose2( dPT, nb, dA, rows, M-ii, nb0);
                  ...
                  if( m0 > 0 ) {      /* if more rows to be factorized */
                    ...
                    if( I > 0 ) {
                      ...
                      cuCtxSynchronize();
                      magmablas_dtranspose2( dA, cols, inAT(I/nb,0), maxn, nb0, cols );
                      cublasGetMatrix( M-I, nb0, sizeof(double), dA, cols, work, lda);
                    }
                    
                    /* factorize the first diagonal block of this big panel; ipiv is 1-base */
                    lapackf77_dgetrf( &m0, &nb0, work, &lda, ipiv+I, &iinfo);
                    if( iinfo != 0 ) {
                      *info = iinfo;
                      break;
                    }
                    ...
                    for( ii = 0; ii < s; ii++ ) 
                      {
                        ...
                        if (ii>0) {
                          ...
                          magmablas_dtranspose( dA, cols, inAT(i,ii), maxn, nb, cols );
                        ...
                        if (s > (ii+1)) {
                          ...
                          cublasDtrsm( MagmaRight, MagmaUpper, MagmaNoTrans, MagmaUnit, N-s*nb, nb,
                                       c_one, inAT(i, ii  ), maxn,
                                       inAT(i, ii+1), maxn);
                          cublasDgemm( MagmaNoTrans, MagmaNoTrans, N-s*nb, M-(i+1)*nb, nb,
                    ...
                    if( nb0 > 0 ) {
                      ...
                      magmablas_dtranspose2( dA, cols, inAT(i,s), maxn, nb0, rows);
                  ...
                  magmablas_dgetmatrix_transpose( M, N, dAT, maxn, A(0,I), lda,  dA, maxm, nb);
        } /* end of for */
...
extern "C" magma_int_t
magma_dgetrf_piv(magma_int_t m, magma_int_t n, double *a, magma_int_t lda, 
                     magma_int_t *ipiv, magma_int_t *info)
{
    magma_int_t nb;
