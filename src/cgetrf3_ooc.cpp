...
#define FLOPS(m, n) (      FMULS_GETRF(m, n) +      FADDS_GETRF(m, n) )
#endif
#endif
    cuFloatComplex    *dAT[4], *dA[4], *dPT[4];
    ...
    *info = 0;
    ...
    for( d=0; d<num_gpus; d++ ) {
      magma_setdevice(d);
      if (MAGMA_SUCCESS != magma_cmalloc( &dA[d], (h*nb + ldn_local)*maxm )) {
    ...
    for( I=0; I<n; I+=NB ) {
          ...
          //magmablas_chtodt4(num_gpus, (cudaStream_t **)stream, A(0,I), lda, 
          ...
          for( offset = 0; offset<min(m,I); offset+=NB ) 
          {
            ...
            for( d=0; d<num_gpus; d++ ) {
              magma_setdevice(d);
              magma_csetmatrix_async( (M-offset), nb,
                                      A(offset,offset), lda,
            ...
            for( jj=0, ib=offset/nb; jj<NBk; jj+=nb, ib++ ) 
            {
              ...
              for( d=0; d<num_gpus; d++ ) {
                ...
                magmablas_ctranspose2( inPT(d,0,0), nb, dA[d], rows, M-ii, nb);
...
extern "C" magma_int_t
magma_cgetrf2_piv(magma_int_t num_gpus0, magma_int_t m, magma_int_t n, cuFloatComplex *a, magma_int_t lda, 
                  magma_int_t *ipiv, magma_int_t *info)
{
