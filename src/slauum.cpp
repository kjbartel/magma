        ...
        else
        {
                if (upper)
                {
                        ...
                        for (i=0; i<n; i=i+nb)
                        {
                                ...
                                //cublasSetMatrix(ib, (n-i), sizeof(float), A(i, i), lda, dA(i, i), ldda);
                                
                                cudaMemcpy2DAsync( dA(i, i), ldda *sizeof(float),
                                                   A(i,i), lda*sizeof(float),
                                                   sizeof(float)*ib, ib,
                                                   cudaMemcpyHostToDevice,stream[1]);

                                cudaMemcpy2DAsync( dA(i,i+ib),  ldda *sizeof(float),
                                                   A(i,i+ib), lda*sizeof(float),
                                                   sizeof(float)*ib, (n-i-ib),
                                                   cudaMemcpyHostToDevice,stream[0]);

                                cudaStreamSynchronize(stream[1]);

                                cublasStrmm( MagmaRight, MagmaUpper,
                                ...
                                if (i+ib < n)
                                {
                                        ...
                                        cublasSsyrk( MagmaUpper, MagmaNoTrans, ib,(n-i-ib),
                                                     done, dA(i, i+ib), ldda,
                                                     done,  dA(i, i), ldda);
                                }
                                
                                cublasGetMatrix( i+ib,ib, sizeof(float),
                ...
                else
                {
                        ...
                        {
                                ib=min(nb,n-i);
                                //cublasSetMatrix((n-i), ib, sizeof(float),
                                ...
                                cudaMemcpy2DAsync( dA(i+ib, i),  ldda *sizeof(float),
                                                   A(i+ib, i), lda*sizeof(float),
                                                   sizeof(float)*(n-i-ib), ib,
                                                   cudaMemcpyHostToDevice,stream[0]);

                                cudaStreamSynchronize(stream[1]);

                                cublasStrmm( MagmaLeft, MagmaLower,
                                ...
                                if (i+ib < n)
                                {
                                        ...
                                        cublasSsyrk(MagmaLower, MagmaTrans, ib, (n-i-ib),
                                                        done, dA(i+ib, i), ldda,
                                                        done,  dA(i, i), ldda);
                                }
                                cublasGetMatrix(ib, i+ib, sizeof(float),
