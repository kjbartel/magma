...
#define inAT(id,i,j) (d_lAT[(id)] + (i)*nb*lddat + (j)*nb)
    ...
    float *d_lAT[4], *d_lAP[4], *d_panel[4], *panel_local[4], *work;
    ...
    *info = 0;
    ...
    nb     = magma_get_sgetrf_nb(m);
        ...
        if (nb <= 1 || nb >= n) {
          ...
          cublasGetMatrix(m, n, sizeof(float), d_lA[0], ldda, work, m);
          lapackf77_sgetrf(&m, &n, work, &m, ipiv, info);
          cublasSetMatrix(m, n, sizeof(float), work, m, d_lA[0], ldda);
          free(work);
    } else {
        ...
        cudaSetDevice(i);
                ...
                /* workspaces */
            if ( CUBLAS_STATUS_SUCCESS != cublasAlloc(nb*maxm, sizeof(float), (void**)&d_lAP[i]) ) {
                  for( j=0; j<i; j++ ) {
            cudaSetDevice(j);
                    cublasFree( d_lAP[j] );
            ...
            if ( CUBLAS_STATUS_SUCCESS != cublasAlloc(nb*maxm, sizeof(float), (void**)&d_panel[i]) ) {
                  for( j=0; j<=i; j++ ) {
            cudaSetDevice(j);
                    cublasFree( d_lAP[j] );
                  }
            ...
            lddat = ldat_local[i];
            if ( CUBLAS_STATUS_SUCCESS != cublasAlloc(lddat*maxm, sizeof(float), (void**)&d_lAT[i]) ) {
                  for( j=0; j<=i; j++ ) {
            cudaSetDevice(j);
                    cublasFree( d_lAP[j] );
                    cublasFree( d_panel[j] );
          ...
          /* cpu workspace */
          lddwork = maxm;
          if ( cudaSuccess != cudaMallocHost( (void**)&work, lddwork*nb*sizeof(float) ) ) {
                for(i=0; i<num_gpus; i++ ) {
          cudaSetDevice(i);
              cublasFree( d_lAP[i] );
          ...
          for( i=0; i<s; i++ )
            {
                ...
                //cublasGetMatrix( m-i*nb, nb, sizeof(float), d_lAP[id], cols, work, lddwork);
                                cudaMemcpy2DAsync( work,     lddwork*sizeof(float),
                                                   d_lAP[id], cols  *sizeof(float),
                                                   sizeof(float)*rows, nb,
                ...
                if ( (*info == 0) && (iinfo > 0) ) {
                    *info = iinfo + i*nb;
                                ...
                                for( d=0; d<num_gpus; d++ ) {
                                  cudaSetDevice(d);
                      lddat = ldat_local[d];
                  //cublasSetMatrix(rows, nb, sizeof(float), work, lddwork, d_lAP[d], maxm);
                  cudaMemcpy2DAsync(d_lAP[d], maxm   *sizeof(float),
                                                    work,     lddwork*sizeof(float),
                                                    sizeof(float)*rows, nb,
                      ...
                      lddat = ldat_local[d];
                                  /* apply the pivoting */
                              if( d == 0 ) 
                    magmablas_spermute_long2( d_lAT[d], lddat, ipiv, nb, i*nb );
                                  else
                    magmablas_spermute_long3( d_lAT[d], lddat, ipiv, nb, i*nb );
            ...
            /* Set the GPU number that holds the last panel */
              ...
              //cudaStreamSynchronize(streaml[id][1]);
                  ...
                  cublasGetMatrix(rows, nb0, sizeof(float), d_lAP[id], maxm, work, lddwork);
                  ...
                  *info = iinfo + s*nb;
                  ...
                  for( d=0; d<num_gpus; d++ ) {
                    ...
                    lddat = ldat_local[d];
                            ...
                            {
                      //cublasSetMatrix(rows, nb0, sizeof(float), work, lddwork, d_lAP[d], maxm);
                  cudaMemcpy2DAsync(d_lAP[d], maxm   *sizeof(float),
                                                work,     lddwork*sizeof(float),
                                                sizeof(float)*rows, nb0,
                                                cudaMemcpyHostToDevice, streaml[d][0]);
                            }
                          }
                        }
                ...
                for( d=0; d<num_gpus; d++ ) {
                  ...
                  lddat = ldat_local[d];
                          ...
                          if( nb0 > 0 ) {
                            if( d == 0 ) 
                                  magmablas_spermute_long2( d_lAT[d], lddat, ipiv, nb0, s*nb );
                            else
                                  magmablas_spermute_long3( d_lAT[d], lddat, ipiv, nb0, s*nb );
