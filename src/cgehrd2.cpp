...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    cuFloatComplex *da;
    cublasStatus status;
    status = cublasAlloc(N*ldda+2*N*nb+nb*nb, sizeof(cuFloatComplex), (void**)&da);
    if (status != CUBLAS_STATUS_SUCCESS) {
      fprintf (stderr, "!!!! device memory allocation error (d_A)\n");
    }
    
    cuFloatComplex *d_A    = da;
    cuFloatComplex *d_work = da + (N+nb)*ldda;
    ...
    if (nb < nbmin || nb >= nh) {
      ...
      cublasSetMatrix(N, N-ilo+1, sizeof(cuFloatComplex), a+(ilo-1)*(lda), lda, d_A, ldda);
      ...
      for (i__ = ilo; i__ < ihi - nb; i__ += nb) {
        ...
        cublasGetMatrix(ihi-i__+1, ib, sizeof(cuFloatComplex), 
        ...
        cublasSetMatrix(nb, nb, sizeof(cuFloatComplex), t, nb, d_t, nb);
    ...
    if (!(nb < nbmin || nb >= nh))
        cublasGetMatrix( n, n-i__+1, sizeof(cuFloatComplex), 
