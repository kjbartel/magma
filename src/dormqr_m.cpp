/*
    -- MAGMA (version 1.2.0) --
       ...
       @generated d Tue May 15 18:17:45 2012
...
extern"C"{
    void magmablas_dsetdiag1subdiag0_stream(char uplo, int k, int nb, double *A, int lda, cudaStream_t stream);
}

extern "C" magma_int_t
magma_dormqr_m(magma_int_t nrgpu, const char side, const char trans,
...
/*  -- MAGMA (version 1.2.0) --
    ...
    *info = 0;
    ...
    for (igpu = 0; igpu < nrgpu; ++igpu){
        ...
        if (MAGMA_SUCCESS != magma_dmalloc( &dw[igpu], (n_l*lddc + 2*lddac*lddar + 2*(nb + 1 + lddwork)*nb) )) {
            printf("%d: size: %ld\n", igpu, (n_l*lddc + 2*lddac*lddar + (nb+1+lddwork)*nb)*sizeof(double));
            magma_xerbla( __func__, -(*info) );
    ...
    else
    {
        ...
        if (left) {
            ...
            for (igpu = 0; igpu < nrgpu; ++igpu){
                magma_setdevice(igpu);
                magma_dsetmatrix_async( (nq-i1), kb,
                                        A(i1, i1),            lda,
            ...
            for (i = i1; i3 < 0 ? i >= i2 : i < i2; i += i3)
            {
                ...
                if (kb > 0 && i+i3 >= 0){
                    for (igpu = 0; igpu < nrgpu; ++igpu){
                        magma_setdevice(igpu);
                        magma_dsetmatrix_async( (i__4-i3), kb,
                                                A(i+i3, i+i3),                    lda,
                ...
                for (igpu = 0; igpu < nrgpu; ++igpu){
                    ...
                    magmablas_dsetdiag1subdiag0_stream('L', ib, ib, dA_c(igpu, ind_c, i, 0), lddac, stream[igpu][ind_c]);
