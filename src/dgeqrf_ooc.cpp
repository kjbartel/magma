        ...
        for(int j=0; j<min(i,k); j+=nb)
          {
            ...
            cudaMemcpy2DAsync(dwork, lddwork *sizeof(double),
                              work,  ib      *sizeof(double),
                              sizeof(double)*ib, ib,
                              cudaMemcpyHostToDevice,stream[1]);

            dpanel_to_q(MagmaUpper, ib, a_ref(j,j), lda, work+ib*ib);
            cudaMemcpy2DAsync(ptr,        rows *sizeof(double),
