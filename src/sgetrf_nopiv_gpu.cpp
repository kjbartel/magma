/*
    -- MAGMA (version 1.1) --
       ...
       @generated s Sun Nov 13 20:48:17 2011
...
#define inA(i,j) (dA + (i)*nb + (j)*nb*ldda)
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) {
        ...
        for( i=0; i<s; i++ )
          {
            // download i-th panel
            cols = maxm - i*nb;
            cublasGetMatrix( m-i*nb, nb, sizeof(float),
            ...
            // upload i-th panel
            cublasSetMatrix(m-i*nb, nb, sizeof(float), work, lddwork, 
                            inA(i, i), ldda);
        ...
        rows = m - s*nb;
        cols = maxm - s*nb;
        cublasGetMatrix(rows, nb0, sizeof(float), inA(s,s), ldda, work, lddwork);
        ...
        // upload i-th panel
        cublasSetMatrix(rows, nb0, sizeof(float), work, lddwork, inA(s,s), ldda);
