...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if( CUBLAS_STATUS_SUCCESS != cublasAlloc(ldtwork, 
                                             sizeof(cuFloatComplex), (void**)&dT) ) {
        return MAGMA_ERR_CUBLASALLOC;
    }
    
    tau = (cuFloatComplex*) malloc( k * sizeof(cuFloatComplex) );
    if( tau == NULL ) {
        cublasFree(dT);
        return MAGMA_ERR_ALLOCATION;
    }

    ret = magma_cgeqrf3_gpu( m, n, dA, ldda, tau, dT, info );
    ...
    ret = magma_cgeqrs3_gpu(m, n, nrhs,
                           dA, ldda, tau, dT, 
