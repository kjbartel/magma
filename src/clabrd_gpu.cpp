...
/*  -- MAGMA (version 1.1) --
    ...
    cuFloatComplex *f = (cuFloatComplex *)malloc(max(n,m)*sizeof(cuFloatComplex ));
    ...
    if (m >= n) {
        ...
        for (i__ = 1; i__ <= nb; ++i__) {
            ...
            if (i__ < n) {
                ...
                cublasSetVector(i__2, sizeof(cuFloatComplex),
                                a + i__   + i__   * a_dim1, 1,
                                da+(i__-1)+(i__-1)* (ldda), 1);
                // 2. Multiply ---------------------------------------------
                cublasCgemv(MagmaConjTrans, i__2, i__3, c_one, 
                            da + (i__-1) + ((i__-1) + 1) * (ldda), ldda, 
                            da + (i__-1) + (i__-1) * (ldda), c__1, c_zero, 
                            dy + i__ + 1 + i__ * y_dim1, c__1);
                
                // 3. Put the result back ----------------------------------
                cudaMemcpy2DAsync(y+i__+1+i__*y_dim1, y_dim1*sizeof(cuFloatComplex),
                                  dy+i__+1+i__*y_dim1, y_dim1*sizeof(cuFloatComplex),
                                  sizeof(cuFloatComplex)*i__3, 1,
                ...
                cublasSetVector(i__3, sizeof(cuFloatComplex),
                                ...
                                da+(i__-1)+((i__-1)+1)*(ldda), ldda);
                // 2. Multiply ---------------------------------------------
                //cublasCcopy(i__3, da+(i__-1)+((i__-1)+1)*(ldda), ldda,
                //            dy + 1 + lddy, 1);
                cublasCgemv('N', i__2, i__3, c_one,
                ...
                // 3. Put the result back ----------------------------------
                cudaMemcpy2DAsync(x+i__+1+i__*x_dim1, x_dim1*sizeof(cuFloatComplex),
                                  dx+i__+1+i__*x_dim1, x_dim1*sizeof(cuFloatComplex),
                                  sizeof(cuFloatComplex)*i__2, 1,
                                  cudaMemcpyDeviceToHost,stream);
      ...
      for (i__ = 1; i__ <= nb; ++i__) {
        ...
        if (i__ < m) {
          ...
          // 2. Multiply ---------------------------------------------
          //cublasCcopy(i__3, da+(i__-1)+(i__-1)*(ldda), ldda,
          //            dy + 1 + lddy, 1);
          cublasCgemv(MagmaNoTrans, i__2, i__3, c_one,
