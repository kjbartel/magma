...
extern "C" void
magmablas_snrm2(int m, int num, float *da, magma_int_t ldda,
                 float *dxnorm);
...
/*  -- MAGMA (version 1.3.0) --
    ...
    *info = 0;
    ...
    k = min(*m,*n);
    magmablas_snrm2(*m, k, da_ref(0,0), *ldda, dwork);

    for (i = 0; i < k; ++i) {

        /*  Generate elementary reflector H(i) to annihilate A(i+1:m,i) */
        magma_slarfg_gpu(*m-i, da_ref(i, i), da_ref(min(i+1,*m), i), dtau+i, dwork+i);
