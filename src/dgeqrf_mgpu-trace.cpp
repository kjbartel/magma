/*
    -- MAGMA (version 1.1) --
       ...
       @generated d Sun Nov 13 20:48:21 2011
...
magma_dgeqrf2_mgpu( int num_gpus, magma_int_t m, magma_int_t n,
...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if ( cudaSuccess != cudaMallocHost( (void**)&local_work, lwork*sizeof(double)) ) {
      *info = -9;
      for(i=0; i<num_gpus; i++){
        #ifdef  MultiGPUs
          cudaSetDevice(i);
        ...
        for (i = 0; i < k-nx; i += nb) 
          {
            ...
            #endif
            cudaMemcpy2DAsync( hwrk_ref(i), ldwork*sizeof(double),
                               dlA(panel_gpunum, i, i_local), ldda*sizeof(double),
            ...
            if (i>0){
                ...
                cudaSetDevice(la_gpu);
                #endif
                cudaMemcpy2DAsync( panel[la_gpu], ldda  *sizeof(double),
                                   hwrk_ref(old_i),  ldwork*sizeof(double),
            ...
            for(j=0; j<num_gpus; j++)
              {  
                ...
                cudaMemcpy2DAsync(panel[j],    ldda  *sizeof(double),
                                  hwrk_ref(i), ldwork*sizeof(double),
                                  sizeof(double)*rows, ib,
                ...
                for(j=0; j<num_gpus; j++)
                  {
                       ...
                       cudaSetDevice(j);
                    #endif
                       cudaMemcpy2DAsync(dwork[j], lddwork *sizeof(double),
                                         lhwrk,    ib      *sizeof(double),
                                         sizeof(double)*ib, ib,
                                         cudaMemcpyHostToDevice, streaml[j][0]);
                  }
