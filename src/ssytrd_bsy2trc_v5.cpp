...
extern "C" {
    ...
    void magma_sbulge_applyQ_v2(char side, magma_int_t NE, magma_int_t N, magma_int_t NB, magma_int_t Vblksiz,
                                float *dE, magma_int_t ldde, float *V, magma_int_t ldv, float *T, magma_int_t ldt, magma_int_t *info);

    void magma_sstedc_withZ(char JOBZ, magma_int_t n, float *D, float * E, float *Z, magma_int_t LDZ);
    void magma_sstedx_withZ(magma_int_t n, magma_int_t ne, float *D, float * E, float *Z, magma_int_t LDZ);

    magma_int_t magma_sorgqr_2stage_gpu(magma_int_t m, magma_int_t n, magma_int_t k, float *da, magma_int_t ldda,
...
static void tile_bulge_parallel(magma_int_t my_core_id, magma_int_t cores_num, float *A, magma_int_t lda,
                                float *V, magma_int_t ldv, float *TAU, magma_int_t n, magma_int_t nb, magma_int_t nb_tiles,
                                magma_int_t band, magma_int_t grsiz, magma_int_t Vblksiz, volatile magma_int_t *prog);

static void tile_bulge_computeT_parallel(magma_int_t my_core_id, magma_int_t cores_num, float *V, magma_int_t ldv, float *TAU,
...
static void tile_bulge_applyQ(magma_int_t wantz, char side, magma_int_t n_cpu, magma_int_t n, magma_int_t nb, magma_int_t Vblksiz,
                              float *E, magma_int_t lde, float *V, magma_int_t ldv, float *TAU, float *T, magma_int_t ldt);
...
public:
    ...
    bulge_data(magma_int_t threads_num_, magma_int_t n_, magma_int_t nb_, magma_int_t nbtiles_,
               ...
               float *T_, magma_int_t ldt_, magma_int_t computeQ1_, float *dQ1_, magma_int_t lddq1_,
               float *dT1_, volatile magma_int_t* prog_)
    :
...
public:
    ...
    applyQ_data(magma_int_t threads_num_, magma_int_t n_, magma_int_t ne_, magma_int_t n_gpu_,
                ...
                float *E_, magma_int_t lde_, float *V_, magma_int_t ldv_, float *TAU_,
...
extern "C" magma_int_t magma_ssytrd_bsy2trc_v5(magma_int_t threads, magma_int_t wantz, char uplo, magma_int_t ne, magma_int_t n, magma_int_t nb,
                                               float *A, magma_int_t lda, float *D, float *E,
                                               /*float* dQ1, magma_int_t lddq1,*/ float *dT1, magma_int_t ldt1)
{
    ////////////////////////////////////////////////////////////////
    float *da, *Z;
    if((wantz>0)/*&&(wantz!=4)*/){
        if (MAGMA_SUCCESS != magma_smalloc( &da, n*lda )) {
    ...
    if(wantz>0)
       magma_ssetmatrix( n, n, A, lda, dQ1, lddq1 );
    ...
    magma_smalloc_cpu(&T,   blkcnt*ldt*Vblksiz);
    magma_smalloc_cpu(&TAU, blkcnt*Vblksiz    );
    magma_smalloc_cpu(&V,   blkcnt*ldv*Vblksiz);
    ...
    if(uplo==MagmaLower){
        ...
        {
            D[i] = MAGMA_S_REAL(A2[i*lda2  ]);
            E[i] = MAGMA_S_REAL(A2[i*lda2+1]);
        }
        D[n-1] = MAGMA_S_REAL(A2[(n-1)*lda2]);
    } else { /* MagmaUpper not tested yet */
        for (magma_int_t i=0; i<n-1; i++)
        {
            D[i]  =  MAGMA_S_REAL(A2[i*lda2+nb]);
            E[i] = MAGMA_S_REAL(A2[i*lda2+nb-1]);
        }
        D[n-1] = MAGMA_S_REAL(A2[(n-1)*lda2+nb]);
    } /* end MagmaUpper */
#else
    if( uplo == MagmaLower ){
        for (magma_int_t i=0; i < n-1; i++) {
    ...
    if(wantz==0){
        ...
        magma_sstedc_withZ(MagmaNoVectors, n, D, E, NULL, 1);
        //magma_sstedc_withZ(MagmaNoVectors, n, D, E, Z, ldz);
        ...
        magma_smalloc_cpu (&Z, n*n);

        magma_sstedx_withZ(n, ne, D, E, Z, ldz);
    ...
    omp_set_num_threads(1);
        ...
        if(wantz==2){
            ...
            if(n_gpu < n)
            {
                ...
                magma_sgetmatrix( n-n_gpu, n, &da[n_gpu], lda, &A[n_gpu], lda);
                ...
                magma_ssetmatrix( n-n_gpu, n, &A[n_gpu], lda, &da[n_gpu], lda);

                /*============================
                 *  use only GPU
                 *==========================*/
            }else{
                magma_sbulge_applyQ_v2('R', n, n, nb, Vblksiz, dQ1, n, V, ldv, T, ldt, &info);
            }
            ...
            float *dZ2;
            if(MAGMA_SUCCESS != magma_smalloc( &dZ2, n*ne )) {
                printf ("!!!! magma_alloc failed for: dZ2\n" );
                exit(-1);
            }
            magma_sgemm( MagmaNoTrans, MagmaNoTrans, n, ne, n, c_one, da, n, dZ, n, c_zero, dZ2, n);
            magma_sgetmatrix( n, ne, dZ2, n, A, lda );
            magma_free(dZ2);
            ...
            if(n_gpu < ne)
            {
                ...
                magma_ssetmatrix(n, ne-n_gpu, Z + n_gpu*ldz, ldz, dZ + n_gpu*ldz, n);
                ...
                magma_ssetmatrix(n, ne, Z, ldz, dZ, n);
                magma_sbulge_applyQ_v2('L', ne, n, nb, Vblksiz, dZ, n, V, ldv, T, ldt, &info);
                magma_device_sync();
            }
            timeaplQ2 = magma_wtime()-timeaplQ2;
            ...
            if (wantz==3){
                ...
                magma_sgemm( MagmaNoTrans, MagmaNoTrans, n, ne, n, c_one, dQ1, lddq1, dZ, n, c_zero, dZ2, n);
                magma_sgetmatrix( n, ne, dZ2, n, A, lda );
                magma_free(dZ2);
                timegemm = magma_wtime()-timegemm;
            }else{
                 ...
                 * apply Q1 to (Q2*Z)
                 * **************************************************/
                printf("calling sormqr\n");
                timegemm = magma_wtime();
                magma_sormqr_gpu_2stages(MagmaLeft, MagmaNoTrans, n-nb, ne, n-nb, dQ1+nb, lddq1,
                                         dZ+nb, n, dT1, nb, &info);
                magma_free(dT1);
                magma_sgetmatrix( n, ne, dZ, n, A, lda );
                timegemm = magma_wtime()-timegemm;
            }
    ...
    if((wantz>0))
    {
        ...
        if(allcores_num==1)
        {
            ...
            if(computeQ1==1){
                ...
                magma_sorgqr_2stage_gpu(n, n, n, dQ1, lddq1, NULL, dT1, nb, &info);
                magma_device_sync();
...
static void *applyQ_parallel_section(void *arg)
{
    ...
    if((wantz==2))
    {
        ...
        if(my_core_id == 0)
        {
            ...
            magma_sbulge_applyQ_v2('R', n, n, nb, Vblksiz, dE, ldde, V, ldv, T, ldt, &info);
            magma_device_sync();
            ...
            float* E_loc = E + n_gpu+ n_loc * (my_core_id-1);
...
static void tile_bulge_applyQ(magma_int_t wantz, char side, magma_int_t n_loc, magma_int_t n, magma_int_t nb, magma_int_t Vblksiz,
                              float *E, magma_int_t lde, float *V, magma_int_t ldv, float *TAU, float *T, magma_int_t ldt)//, magma_int_t* info)
{
    //%===========================
    //%   local variables
    //%===========================
