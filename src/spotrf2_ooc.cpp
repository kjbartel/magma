/*
    -- MAGMA (version 1.1) --
       ...
       @generated s Sun Nov 13 20:48:13 2011
...
magma_shtodpo(int num_gpus, char *uplo, magma_int_t m, magma_int_t n, magma_int_t off_i, magma_int_t off_j, magma_int_t nb,
              float *h_A, magma_int_t lda, float **d_lA, magma_int_t ldda, cudaStream_t **stream,
              magma_int_t *info);

extern "C" magma_int_t 
magma_sdtohpo(int num_gpus, char *uplo, magma_int_t m, magma_int_t n, magma_int_t off_i, magma_int_t off_j, magma_int_t nb, magma_int_t NB,
              float *a, magma_int_t lda, float **work, magma_int_t ldda, cudaStream_t **stream,
              magma_int_t *info);

extern "C" magma_int_t
magma_spotrf2_mgpu(int num_gpus, char uplo, magma_int_t m, magma_int_t n, magma_int_t off_i, magma_int_t off_j, magma_int_t nb,
                   float **d_lA, magma_int_t ldda, float **d_lP, magma_int_t lddlp, 
                   float *work, magma_int_t ldwrk, cudaStream_t **streaml, magma_int_t *info);
...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    for (d=0; d<num_gpus; d++ ) {
      cudaSetDevice(d);
      if (CUBLAS_STATUS_SUCCESS != cublasAlloc(NB*lddla+nb*ldda, sizeof(float), (void**)&dt[d])) {
        *info = -6;
    ...
    cudaSetDevice(0);
    ldwrk = n;
    if (cudaSuccess != cudaMallocHost( (void**)&work, ldwrk*nb*sizeof(float) ) ) {
      *info = -6;
    ...
    if (upper) {
      ...
      for( J=0; J<n; J+=NB ) {
        ...
        for( j=0; j<J; j+=nb ) {
          ...
          for( d=0; d<num_gpus; d++ ) {
            cudaSetDevice(d);
            cudaMemcpy2DAsync( dTup(d, 0, J), nb *sizeof(float), 
          ...
          for( jj=J+JB; jj<n; jj+=nb ) {
            ...
            jb2 = min(nb, n-jj);
            cudaMemcpy2DAsync( dTup(d, 0, J+JB+n_local[d]), nb *sizeof(float), 
                               A(j, jj),                    lda*sizeof(float), 
                               sizeof(float)*nb, jb2, 
                               cudaMemcpyHostToDevice,stream[d][0]);
            n_local[d] += jb2;
          }
          ...
          for( jj=0; jj<JB; jj+=nb ) { /* diagonal */
            ...
            cublasSsyrk(MagmaUpper, MagmaTrans, jb2, jb3,
        ...
        magma_spotrf2_mgpu(num_gpus, uplo, JB, n-J, J, J, nb, dwork, NB, dt, ldda, a, lda, (cudaStream_t **)stream, &iinfo);
        if( iinfo != 0 ) {
            *info = J+iinfo;
            break;
        }
        ...
        //if( J+JB < n )
        //cudaMemcpy2DAsync(  A  (   J, J+JB), lda*sizeof(float),
        //                   dAup(d, 0, J+JB), NB *sizeof(float),
        //                   sizeof(float)*JB, n-J-JB, 
        //                   cudaMemcpyDeviceToHost,stream[2]);
        //magma_sdtohpo(num_gpus, &uplo, JB, n, J, J, nb, a, lda, dwork, NB, (cudaStream_t **)stream, &iinfo);
      ...
      for( J=0; J<n; J+=NB ) {
        ...
        for( j=0; j<J; j+=nb ) {
          ...
          for( d=0; d<num_gpus; d++ ) {
            cudaSetDevice(d);
            cudaMemcpy2DAsync( dT(d, J, 0), ldda *sizeof(float), 
                               A(J, j),     lda  *sizeof(float), 
                               sizeof(float)*JB, nb, 
                               cudaMemcpyHostToDevice,stream[d][0]);
            n_local[d] = 0;
          ...
          for( jj=J+JB; jj<n; jj+=nb ) {
            ...
            jb2 = min(nb, n-jj);
            cudaMemcpy2DAsync( dT(d, J+JB+n_local[d], 0), ldda *sizeof(float), 
                               A(jj, j),                  lda*sizeof(float), 
                               sizeof(float)*jb2, nb, 
                               cudaMemcpyHostToDevice,stream[d][0]);
            n_local[d] += jb2;
          }
          ...
          for( jj=0; jj<JB; jj+=nb ) { /* diagonal */
            ...
            cublasSsyrk(MagmaLower, MagmaNoTrans, jb2, jb3,
        ...
        /* factor the big panel */
        magma_spotrf2_mgpu(num_gpus, uplo, n-J, JB, J, J, nb, dwork, lddla, dt, ldda, a, lda, (cudaStream_t **)stream, &iinfo);
        if( iinfo != 0 ) {
            *info = J+iinfo;
            break;
        }
        ...
        //if( J+JB < n )
        //cudaMemcpy2DAsync(  A(J+JB, J),    lda *sizeof(float),
        //                   dA(d, J+JB, 0), ldda*sizeof(float),
        //                   sizeof(float)*(n-J-JB), JB,
