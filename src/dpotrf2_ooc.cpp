...
extern "C" magma_int_t
magma_dpotrf3_mgpu(int num_gpus, char uplo, magma_int_t m, magma_int_t n, magma_int_t off_i, magma_int_t off_j, magma_int_t nb,
...
//#define ROW_MAJOR_PROFILE
    ...
    *info = 0;
    ...
    for (d=0; d<num_gpus; d++ ) {
      magma_setdevice(d);
      if (MAGMA_SUCCESS != magma_dmalloc( &dt[d], NB*lddla + 2*nb*ldda )) {
    ...
    magma_setdevice(0);
    ldwrk = n;
    if (MAGMA_SUCCESS != magma_dmalloc_pinned( &work, ldwrk*nb )) {
      ...
      for( J=0; J<n; J+=NB ) {
        ...
        for( j=0; j<J; j+=nb ) {
          ...
          for( jj=0; jj<JB; jj+=nb ) { /* diagonal */
            ...
            magma_dsyrk(MagmaUpper, MagmaTrans, jb2, jb3,
        ...
        magma_dpotrf3_mgpu(num_gpus, uplo, JB, n-J, J, J, nb, dwork, NB, dt, ldda, a, lda, (cudaStream_t **)stream, &iinfo);
        if( iinfo != 0 ) {
            *info = J+iinfo;
            break;
        }
      ...
      for( J=0; J<n; J+=NB ) {
        ...
        for( j=0; j<J; j+=nb ) {
          ...
          for( jj=0; jj<JB; jj+=nb ) { /* diagonal */
            ...
            magma_dsyrk(MagmaLower, MagmaNoTrans, jb2, jb3,
        ...
        /* factor the big panel */
        magma_dpotrf3_mgpu(num_gpus, uplo, n-J, JB, J, J, nb, dwork, lddla, dt, ldda, a, lda, (cudaStream_t **)stream, &iinfo);
        if( iinfo != 0 ) {
            *info = J+iinfo;
            break;
        }
        ...
        magma_ddtohpo( num_gpus, &uplo, n, JB, J, J, nb, JB, a, lda, dwork, lddla, (cudaStream_t **)stream, &iinfo);
