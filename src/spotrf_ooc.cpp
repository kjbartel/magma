...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if (CUBLAS_STATUS_SUCCESS != cublasAlloc((NB+nb)*ldda, sizeof(float), (void**)&dt)) {
          *info = -6;
    ...
    if (nb <= 1 || nb >= n) {
        ...
        //                A(J, J), lda, dAup(0,J), NB);
            ...
            for (jj=0; jj<JB; jj+=nb) {
              ...
              cublasSsyrk(MagmaUpper, MagmaTrans, jb, jj, 
                          mdone, dAup(0,  j), NB, 
                          done,  dAup(jj, j), NB);

                  /* send the diagonal-block to CPU */
              cudaMemcpy2DAsync(  A  (J, j), lda*sizeof(float), 
                                 dAup(0, j), NB *sizeof(float), 
                                 sizeof(float)*(jj+jb), jb,
                                 cudaMemcpyDeviceToHost, stream[1]);
              //cudaMemcpy2DAsync(  A  ( j, j), lda*sizeof(float), 
              //                   dAup(jj, j), NB *sizeof(float), 
              //                   sizeof(float)*jb, jb,
              //                   cudaMemcpyDeviceToHost, stream[1]);
              ...
              if ( (j+jb) < n ) {
                    ...
                    cudaMemcpy2DAsync(dAup(jj, j), NB  * sizeof(float), 
          ...
          for( J=0; J<n; J+=NB ) {
            ...
            for (jj=0; jj<JB; jj+=nb) {
              ...
              cublasSetMatrix((n-J), nb, sizeof(float), 
                              A(J, j), lda, dT(J, 0), ldda);

                  /* update the current big-panel    *
                   * using the previous block-column */
              cublasSsyrk(MagmaLower, MagmaNoTrans, JB, nb,
                          mdone, dT(J, 0), ldda, 
                          done,  dA(J, 0), ldda);
                  if( J+JB < n )
            ...
            for (jj=0; jj<JB; jj+=nb) {
              ...
              /* Update the current diagonal block */
              cublasSsyrk(MagmaLower, MagmaNoTrans, jb, jj,
                          mdone, dA(j, 0), ldda, 
                          done,  dA(j, jj), ldda);
              ...
              if ( (j+jb) < n) {
                    /* send the diagonal-block to GPU */
                cudaMemcpy2DAsync( dA(j, jj), ldda*sizeof(float), 
                                   A(j,   j), lda *sizeof(float), 
                                   sizeof(float)*jb, jb, 
                                   cudaMemcpyHostToDevice,stream[0]);
