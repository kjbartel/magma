...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    for (d=0; d<num_gpus; d++ ) {
      cudaSetDevice(d);
      if (CUBLAS_STATUS_SUCCESS != cublasAlloc(NB*lddla+nb*ldda, sizeof(cuFloatComplex), (void**)&dt[d])) {
        *info = -6;
    ...
    cudaSetDevice(0);
    ldwrk = n;
    if (cudaSuccess != cudaMallocHost( (void**)&work, ldwrk*nb*sizeof(cuFloatComplex) ) ) {
      *info = -6;
    ...
    if (upper) {
      ...
      for( J=0; J<n; J+=NB ) {
        ...
        for( j=0; j<J; j+=nb ) {
          ...
          for( d=0; d<num_gpus; d++ ) {
            cudaSetDevice(d);
            cudaMemcpy2DAsync( dTup(d, 0, J), nb *sizeof(cuFloatComplex), 
                               A(j, J),       lda*sizeof(cuFloatComplex), 
                               sizeof(cuFloatComplex)*nb, JB, 
          ...
          for( jj=J+JB; jj<n; jj+=nb ) {
            ...
            jb2 = min(nb, n-jj);
            cudaMemcpy2DAsync( dTup(d, 0, J+JB+n_local[d]), nb *sizeof(cuFloatComplex), 
                               A(j, jj),                    lda*sizeof(cuFloatComplex), 
                               sizeof(cuFloatComplex)*nb, jb2, 
                               cudaMemcpyHostToDevice,stream[d][0]);
            n_local[d] += jb2;
          }
          for( d=0; d<num_gpus; d++ ) {
          ...
          for( jj=0; jj<JB; jj+=nb ) { /* diagonal */
            ...
            cublasCherk(MagmaUpper, MagmaConjTrans, jb2, jb3,
        ...
        magma_cpotrf2_mgpu(num_gpus, uplo, JB, n-J, J, J, nb, dwork, NB, dt, ldda, a, lda, (cudaStream_t **)stream, &iinfo);
        if( iinfo != 0 ) {
            *info = J+iinfo;
            break;
        }
        ...
        //if( J+JB < n )
        //cudaMemcpy2DAsync(  A  (   J, J+JB), lda*sizeof(cuFloatComplex),
        //                   dAup(d, 0, J+JB), NB *sizeof(cuFloatComplex),
        //                   sizeof(cuFloatComplex)*JB, n-J-JB, 
        //                   cudaMemcpyDeviceToHost,stream[2]);
        //magma_cdtohpo(num_gpus, &uplo, JB, n, J, J, nb, a, lda, dwork, NB, (cudaStream_t **)stream, &iinfo);
      ...
      for( J=0; J<n; J+=NB ) {
        ...
        for( j=0; j<J; j+=nb ) {
          ...
          for( d=0; d<num_gpus; d++ ) {
            cudaSetDevice(d);
            cudaMemcpy2DAsync( dT(d, J, 0), ldda *sizeof(cuFloatComplex), 
                               A(J, j),     lda  *sizeof(cuFloatComplex), 
                               sizeof(cuFloatComplex)*JB, nb, 
                               cudaMemcpyHostToDevice,stream[d][0]);
            n_local[d] = 0;
          }
          ...
          for( jj=J+JB; jj<n; jj+=nb ) {
            ...
            jb2 = min(nb, n-jj);
            cudaMemcpy2DAsync( dT(d, J+JB+n_local[d], 0), ldda *sizeof(cuFloatComplex), 
                               A(jj, j),                  lda*sizeof(cuFloatComplex), 
                               sizeof(cuFloatComplex)*jb2, nb, 
                               cudaMemcpyHostToDevice,stream[d][0]);
            n_local[d] += jb2;
          }
          for( d=0; d<num_gpus; d++ ) {
          ...
          for( jj=0; jj<JB; jj+=nb ) { /* diagonal */
            ...
            cublasCherk(MagmaLower, MagmaNoTrans, jb2, jb3,
        ...
        /* factor the big panel */
        magma_cpotrf2_mgpu(num_gpus, uplo, n-J, JB, J, J, nb, dwork, lddla, dt, ldda, a, lda, (cudaStream_t **)stream, &iinfo);
        if( iinfo != 0 ) {
            *info = J+iinfo;
            break;
        }
        ...
        //if( J+JB < n )
        //cudaMemcpy2DAsync(  A(J+JB, J),    lda *sizeof(cuFloatComplex),
        //                   dA(d, J+JB, 0), ldda*sizeof(cuFloatComplex),
        //                   sizeof(cuFloatComplex)*(n-J-JB), JB,
