/*
    -- MAGMA (version 1.1) --
       ...
       @generated s Sun Nov 13 20:48:23 2011
...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    if ( CUBLAS_STATUS_SUCCESS 
         != cublasAlloc(n*ldda+(m+n)*nb, sizeof(float), (void**)&da) ) {
    ...
    for (i=0; i< (minmn - nx); i += nb) {
        ...
        if ( i > 0 ) {
            cublasGetMatrix(nrow, nb, sizeof(float),
                            dA(i, i), ldda, 
                            A( i, i), lda );
            cublasGetMatrix(nb, ncol - nb, sizeof(float),
                            dA(i, i+nb), ldda,
                            A( i, i+nb), lda);
        }
        ...
        cublasSetMatrix(nrow, nb, sizeof(float),
                        work  + nb, ldwrkx,
                        dwork + nb, ldwrkx);
        cublasSetMatrix(ncol, nb, sizeof(float),
    ...
    if ( 0 < (minmn-nx) )
        cublasGetMatrix(nrow, ncol, sizeof(float),
                        dA(i, i), ldda,
                        A( i, i), lda);
