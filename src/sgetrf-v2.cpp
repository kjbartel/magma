...
extern "C" magma_int_t
magma_sgetrf2(magma_int_t m, magma_int_t n, float *a, magma_int_t lda, 
              magma_int_t *ipiv, magma_int_t *info)
{
/*  -- MAGMA (version 1.1) --
       Univ. of Tennessee, Knoxville
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        if (CUBLAS_STATUS_SUCCESS != cublasAlloc((2*nb+maxn)*maxm, 
          ...
          magma_sgetrf_piv( m, n, a, lda, ipiv, info);
          return rval;
        }
        dAT = dA + 2*nb*maxm; 
        magmablas_ssetmatrix_transpose( m, n-nb, a+nb*lda, lda, dAT+nb, ldda, dA, maxm, nb);
        ...
        if (n>=m){
          ...
          magmablas_sgetmatrix_transpose( m, n, dAT, ldda, a, lda, dA, maxm, nb);
        } else {
          magmablas_stranspose2( dA, maxm, inAT(0,s), ldda, nb0, m);
          cublasGetMatrix(m, nb0, sizeof(float), dA, maxm, a+s*nb*lda, lda);
          
          // make sure that gpu queue is empty
          cuCtxSynchronize();
          
          // do the cpu part
          lapackf77_sgetrf( &rows, &nb0, a+s*nb+s*nb*lda, &lda, ipiv+s*nb, &iinfo);
          if (*info == 0 && iinfo > 0)
            *info = iinfo + s*nb;
          magmablas_spermute_long2( dAT, ldda, ipiv, nb0, s*nb );
          
          magmablas_sgetmatrix_transpose( m, n-nb0, dAT, ldda, a, lda, dA, maxm, nb);
        }
