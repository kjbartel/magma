...
/*  -- MAGMA (version 1.2.1) --
    ...
    Purpose   
    =======   
    DSYBBD reduces a real symmetric matrix A to real symmetric   
    ...
    int ldda = ((n+31)/32)*32; // magma_get_dsybbd_nb(n); 
    int lddt = nb;
   
    double c_neg_one  = MAGMA_D_NEG_ONE, c_neg_half;
    MAGMA_D_DSCALE(c_neg_half, c_neg_one, 2.);
    ...
    *info = 0;
    ...
    if (upper) {
      ...
      printf("DSYBBD is not yet implemented for upper matrix storage. Exit.\n");
      exit(1);
