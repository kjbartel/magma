...
//
extern "C" void 
magmablas_csetmatrix_transpose( int m, int n,
                                ...
                                cuFloatComplex  *dB, int lddb, int nb )
{
    ...
    ib   = min(n-i, nb);
    cudaMemcpy2DAsync(dB + (j%2) * nb * lddb, lddb*sizeof(cuFloatComplex),
...
//===========================================================================
extern "C" void 
magmablas_csetmatrix_transpose2( int m, int n,
                                 cuFloatComplex  *ha,  int  lda, 
                                 cuFloatComplex **dat, int *ldda,
                                 cuFloatComplex **dB,  int  lddb, int nb,
                                 int num_gpus, cudaStream_t stream[][2] )
{
    ...
    if (n<num_gpus*nb){
       for(i=0; i<n; i+=nb){
          ...
          ib = min(n-i, nb);
          cudaMemcpy2DAsync(dB[k], lddb*sizeof(cuFloatComplex),
                            ha+i*lda, lda*sizeof(cuFloatComplex),
                            sizeof(cuFloatComplex)*m, ib,
    ...
    else
    {
      for(i=0; i<(n + num_gpus*nb); i+=nb){
         ...
         if (i<n){
            /* Move data from CPU to GPU in the second panel in the dB buffer */
            ib = min(n-i, nb);
            cudaMemcpy2DAsync(dB[k] + (j[k]%2)*nb*lddb, lddb*sizeof(cuFloatComplex),
                         ha+i*lda, lda*sizeof(cuFloatComplex),
                         sizeof(cuFloatComplex)*m, ib, 
