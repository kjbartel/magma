...
extern "C" void 
magmablas_dinplace_transpose( double *A, magma_int_t lda, magma_int_t n )
{
        ...
        {
                dim3 grid( in, in/2+1 );
                dinplace_T_odd<<< grid, threads, 0, magma_stream >>>( A, lda, in/2+1 );
        }
        else
        {
                dim3 grid( in+1, in/2 );
                dinplace_T_even<<< grid, threads, 0, magma_stream >>>( A, lda, in/2 );
        }
}
