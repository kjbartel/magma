    ...
    magma_int_t ngpu, magma_int_t nb, cudaStream_t streams[][20], magma_int_t nstream )
{
    ...
    for( magma_int_t d = 0; d < ngpu; ++d ) {
        magma_setdevice( d );
        cudaMemset( dC[d], 0, lddc*n*sizeof(cuFloatComplex) );
    ...
    for( magma_int_t i = 0; i < m; i += nb ) {
        ...
        magmablas_csymmetrize( MagmaLower, ib, dA(dev,ioff,di), ldda );
    }
    for( magma_int_t i = 0; i < m; i += nb ) {
        magma_int_t ib     = min( nb, m-i );      // block size
        magma_int_t ioff   = i + offset;          // start global index in parent matrix
        ...
        magma_cgemm( MagmaConjTrans, MagmaNoTrans, ib, n, m-i,
    ...
    for( magma_int_t i = 0; i < m; i += nb ) {
        ...
        magma_cgemm( MagmaNoTrans, MagmaNoTrans, m-i-ib, n, ib,
    ...
    for( magma_int_t j = 0; j < n; ++j ) {
        blasf77_cscal( &m, &beta, C(0,j), &ione );
    }
    
    // wait and reduce results
    magma_int_t size = ldc*n;
    cuFloatComplex *Ctmp = C(0,n);
    for( magma_int_t d = 0; d < ngpu; ++d ) {
        magma_setdevice( d );
        magma_cgetmatrix( m, n, dC[d], lddc, Ctmp, ldc );
        blasf77_caxpy( &size, &c_one, Ctmp, &ione, C, &ione );
    }
