    ...
    magma_int_t ngpu, magma_int_t nb, cudaStream_t streams[][20], magma_int_t nstream )
{
    ...
    for( magma_int_t d = 0; d < ngpu; ++d ) {
        magma_setdevice( d );
        cudaMemset( dC[d], 0, lddc*n*sizeof(double) );
    ...
    for( magma_int_t i = 0; i < m; i += nb ) {
        ...
        magmablas_dsymmetrize( MagmaLower, ib, dA(dev,ioff,di), ldda );
    }
    for( magma_int_t i = 0; i < m; i += nb ) {
        magma_int_t ib     = min( nb, m-i );      // block size
        magma_int_t ioff   = i + offset;          // start global index in parent matrix
        ...
        magma_dgemm( MagmaTrans, MagmaNoTrans, ib, n, m-i,
    ...
    for( magma_int_t i = 0; i < m; i += nb ) {
        ...
        magma_dgemm( MagmaNoTrans, MagmaNoTrans, m-i-ib, n, ib,
    ...
    for( magma_int_t j = 0; j < n; ++j ) {
        blasf77_dscal( &m, &beta, C(0,j), &ione );
    }
    ...
    for( magma_int_t d = 0; d < ngpu; ++d ) {
        magma_setdevice( d );
        magma_dgetmatrix( m, n, dC[d], lddc, Ctmp, ldc );
        blasf77_daxpy( &size, &c_one, Ctmp, &ione, C, &ione );
    }
