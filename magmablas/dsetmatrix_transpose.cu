/*
    -- MAGMA (version 1.1) --
       ...
       @generated d Sun Nov 13 20:48:40 2011
                                ...
                                double  *dB, int lddb, int nb )
{
    ...
    ib   = min(n-i, nb);
    cudaMemcpy2DAsync(dB + (j%2) * nb * lddb, lddb*sizeof(double),
                                 ...
                                 int num_gpus, cudaStream_t stream[][2] )
{
    ...
    if (n<num_gpus*nb){
       for(i=0; i<n; i+=nb){
          ...
          ib = min(n-i, nb);
          cudaMemcpy2DAsync(dB[k], lddb*sizeof(double),
                            ha+i*lda, lda*sizeof(double),
                            sizeof(double)*m, ib,
    ...
    else
    {
      for(i=0; i<(n + num_gpus*nb); i+=nb){
         ...
         if (i<n){
            /* Move data from CPU to GPU in the second panel in the dB buffer */
            ib = min(n-i, nb);
            cudaMemcpy2DAsync(dB[k] + (j[k]%2)*nb*lddb, lddb*sizeof(double),
                         ha+i*lda, lda*sizeof(double),
                         sizeof(double)*m, ib, 
