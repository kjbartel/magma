...
*/
__global__ void
ssymmetrize_kernel_lower( int m, float *A, int lda )
{
...
__global__ void
ssymmetrize_kernel_upper( int m, float *A, int lda )
{
    // A iterates across row i and AT iterates down column i.
    int i = blockIdx.x*64 + threadIdx.x;
    float *AT = A;
...
/*
  ...
  =====================================================================   */
    ...
    if ( (uplo == 'U') || (uplo == 'u') ) {
        ssymmetrize_kernel_upper<<< grid, threads, 0, magma_stream >>>( m, A, lda );
    }
    else if ( (uplo == 'L') || (uplo == 'l') ) {
        ssymmetrize_kernel_lower<<< grid, threads, 0, magma_stream >>>( m, A, lda );
    }
    else {
