 ...
 *     November 2012
 *
 * @generated d Wed Nov 14 22:54:14 2012
 *
 **/
/* includes, system */
#include <stdlib.h>
...
extern "C" magma_int_t
magma_dpotrf1_mgpu(int num_gpus, char uplo, magma_int_t n,
                      double **d_lA, magma_int_t ldda, magma_int_t *info);
...
int main( int argc, char** argv) 
{
    ...
    if (argc != 1){
        ...
        printf("\nDefault: \n");
        printf("  testing_dpotrf_mgpu -N %d:%d -NGPU %d -UPLO L\n\n", (int) size[0], (int) size[n_sizes-1], (int) num_gpus0 );
    }
    if( N <= 0 || num_gpus0 <= 0 )  {
    ...
    for(i=0; i<num_gpus0; i++){
        magma_setdevice(i);
        TESTING_DEVALLOC( d_lA[i], double, ldda );
    }
    magma_setdevice(0);
