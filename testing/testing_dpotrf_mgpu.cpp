 ...
 *     June 2012
 *
 * @generated d Thu Jun 28 12:31:38 2012
 *
 **/
/* includes, system */
#include <stdlib.h>
...
int main( int argc, char** argv) 
{
    ...
    if (argc != 1){
          ...
          printf("\n  testing_dpotrf_gpu -N %d -NGPU %d -UPLO L\n\n", (int) N, (int) num_gpus0 );
          else
          printf("\n  testing_dpotrf_gpu -N %d -NGPU %d -UPLO U\n\n", (int) N, (int) num_gpus0 );
    } else {
          printf("\nDefault: \n");
          printf("  testing_dpotrf_gpu -N %d:%d -NGPU %d -UPLO L\n\n", (int) size[0], (int) size[n_sizes-1], (int) num_gpus0 );
    }
        if( N <= 0 || num_gpus0 <= 0 )  {
    ...
    for(i=0; i<n_sizes; i++){
      ...
      if( num_gpus0 > N/nb ) {
          ...
          /* call magma_dpotrf_mgp */
             start = get_current_time();
          magma_dpotrf_mgpu(num_gpus, uplo[0], N, d_lA, ldda, &info);
          end = get_current_time();
          if (info < 0) {
                printf("Argument %d of magma_dpotrf_mgpu had an illegal value.\n", (int) -info);
                break;
          } else if (info != 0) {
                printf("magma_dpotrf_mgpu returned info=%d\n", (int) info );
                break;
          }
      gpu_perf = flops / GetTimerValue(start, end);
        ...
        /* =====================================================================
          ...
          if (info < 0) {
                ...
                printf("lapackf77_dpotrf returned info=%d\n", (int) info );
                break;
          }
