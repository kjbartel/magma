 ...
 *     November 2012
 *
 * @generated c Wed Nov 14 22:54:11 2012
 *
 **/
// includes, system
#include <stdlib.h>
...
static int diff_matrix( magma_int_t m, magma_int_t n, cuFloatComplex *A, magma_int_t lda, cuFloatComplex *B, magma_int_t ldb )
{
...
static void init_matrix( magma_int_t m, magma_int_t n, cuFloatComplex *A, magma_int_t lda, magma_int_t offset )
{
    assert( lda >= m );
    for( magma_int_t j = 0; j < n; ++j ) {
        for( magma_int_t i=0; i < m; ++i ) {
...
int main( int argc, char** argv) 
{
    ...
    cuFloatComplex *h_A1, *h_A2;
    cuFloatComplex *d_A1, *d_A2;
    cuFloatComplex *h_R1, *h_R2;
    ...
    TESTING_HOSTALLOC( h_A1, cuFloatComplex, lda*N );
    TESTING_HOSTALLOC( h_A2, cuFloatComplex, lda*N );
    TESTING_HOSTALLOC( h_R1, cuFloatComplex, lda*N );
    TESTING_HOSTALLOC( h_R2, cuFloatComplex, lda*N );
    ...
    TESTING_DEVALLOC( d_A1, cuFloatComplex, ldda*N );
    TESTING_DEVALLOC( d_A2, cuFloatComplex, ldda*N );
    
    magma_queue_t queue = 0;
    
    printf( "npivots=%d\n", npivots );
    printf("        cublasCswap       cswap             cswapblk          claswp   cpermute claswp2  claswpx           ccopymatrix                \n");
    ...
    for( int cnt = 0; cnt < count; ++cnt ) {
        ...
        gbytes = 2. * sizeof(cuFloatComplex) * npivots * N / 1e9;
        ...
        time = magma_sync_wtime( queue );
        magmablas_cswapblk( 'R', N, d_A1, ldda, d_A2, ldda, 1, npivots, ipiv, 1, 0);
        time = magma_sync_wtime( queue ) - time;
        ...
        time = magma_sync_wtime( queue );
        magmablas_cswapblk( 'C', N, d_A1, ldda, d_A2, ldda, 1, npivots, ipiv, 1, 0);
        time = magma_sync_wtime( queue ) - time;
        ...
        /* Row Major */
        memcpy( ipiv2, ipiv, npivots*sizeof(magma_int_t) );  // cpermute updates ipiv2
        init_matrix( N, N, h_A1, lda, 0 );
        magma_csetmatrix( N, N, h_A1, lda, d_A1, ldda );

        time = magma_sync_wtime( queue );
        magmablas_cpermute_long2( N, d_A1, ldda, ipiv2, npivots, 0 );
        time = magma_sync_wtime( queue ) - time;
        ...
        time = magma_sync_wtime( queue );
        magmablas_claswp( N, d_A1, ldda, 1, npivots, ipiv, 1);
        time = magma_sync_wtime( queue ) - time;
        ...
        magmablas_claswp2( N, d_A1, ldda, 1, npivots, d_ipiv );
        time = magma_sync_wtime( queue ) - time;
        ...
        time = magma_sync_wtime( queue );
        magmablas_claswpx( N, d_A1, ldda, 1, 1, npivots, ipiv, 1);
        time = magma_sync_wtime( queue ) - time;
        ...
        time = magma_sync_wtime( queue );
        magmablas_claswpx( N, d_A1, 1, ldda, 1, npivots, ipiv, 1);
        time = magma_sync_wtime( queue ) - time;
        col_perf5 = gbytes / time;

        lapackf77_claswp( &N, h_A1, &lda, &ione, &npivots, ipiv, &ione);
        magma_cgetmatrix( N, N, d_A1, ldda, h_R1, lda );
        check += diff_matrix( N, N, h_A1, lda, h_R1, lda )*shift;
        shift *= 2;
        ...
        time = magma_sync_wtime( queue );
        magma_ccopymatrix( N, npivots, d_A1, ldda, d_A2, ldda );
        time = magma_sync_wtime( queue ) - time;
        // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
        col_perf6 = 0.5 * gbytes / time;

        time = magma_sync_wtime( queue );
        magma_ccopymatrix( npivots, N, d_A1, ldda, d_A2, ldda );
        time = magma_sync_wtime( queue ) - time;
        // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
