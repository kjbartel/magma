...
extern "C" magma_int_t
magma_cpotrf1_mgpu(int num_gpus, char uplo, magma_int_t n,
                      cuFloatComplex **d_lA, magma_int_t ldda, magma_int_t *info);
...
int main( int argc, char** argv) 
{
    ...
    if (argc != 1){
        ...
        printf("\n  testing_cpotrf_mgpu -N %d -NGPU %d -UPLO L\n\n", (int) N, (int) num_gpus0 );
        else
        printf("\n  testing_cpotrf_mgpu -N %d -NGPU %d -UPLO U\n\n", (int) N, (int) num_gpus0 );
    } else {
        printf("\nDefault: \n");
        printf("  testing_cpotrf_mgpu -N %d:%d -NGPU %d -UPLO L\n\n", (int) size[0], (int) size[n_sizes-1], (int) num_gpus0 );
    }
    if( N <= 0 || num_gpus0 <= 0 )  {
    ...
    for(i=0; i<num_gpus0; i++){
        magma_setdevice(i);
        TESTING_DEVALLOC( d_lA[i], cuFloatComplex, ldda );
    }
    magma_setdevice(0);
