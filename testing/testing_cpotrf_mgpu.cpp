...
int main( int argc, char** argv) 
{
    ...
    if (argc != 1){
          ...
          printf("\n  testing_cpotrf_gpu -N %d -NGPU %d -UPLO L\n\n", N, num_gpus0 );
          else
          printf("\n  testing_cpotrf_gpu -N %d -NGPU %d -UPLO U\n\n", N, num_gpus0 );
    } else {
          printf("\nDefault: \n");
          printf("  testing_cpotrf_gpu -N %d:%d -NGPU %d -UPLO L\n\n", size[0],size[n_sizes-1], num_gpus0 );
    }
        if( N <= 0 || num_gpus0 <= 0 )  {
    ...
    for(i=0; i<n_sizes; i++){
      ...
      if( num_gpus0 > N/nb ) {
          ...
          if( lapackf77_lsame(uplo, "U") ) {
            for(j=0; j<N; j+=nb){
              ...
              cudaSetDevice(k);
              nk = min(nb, N-j);
              cublasSetMatrix( N, nk, sizeof(cuFloatComplex), h_A+j*lda, lda,
            ...
            for(j=0; j<N; j+=nb){
              ...
              cudaSetDevice(k);
              nk = min(nb, N-j);
              cublasSetMatrix( nk, N, sizeof(cuFloatComplex), h_A+j, lda,
                               d_lA[k]+j/(nb*num_gpus)*nb, ldn_local);
            }
          }
          cudaSetDevice(0);

          /* call magma_cpotrf_mgp */
             start = get_current_time();
          magma_cpotrf_mgpu(num_gpus, uplo[0], N, d_lA, ldda, &info);
          end = get_current_time();
          if (info < 0) {
        printf("Argument %d of magma_cpotrf_mgpu had an illegal value.\n", -info);
                break;
          } else if (info != 0) {
                printf("magma_cpotrf_mgpu returned info=%d\n",info );
                break;
          }
      gpu_perf = flops / GetTimerValue(start, end);
        ...
        for(j=0; j<N; j+=nb){
              ...
              cudaSetDevice(k);
              nk = min(nb, N-j);
              cublasGetMatrix( N, nk, sizeof(cuFloatComplex),
                               d_lA[k]+j/(nb*num_gpus)*nb*ldda, ldda,
                               h_R+j*lda, lda);
                }
          } else {
            for(j=0; j<N; j+=nb){
              ...
              cudaSetDevice(k);
              nk = min(nb, N-j);
              cublasGetMatrix( nk, N, sizeof(cuFloatComplex), 
                               d_lA[k]+j/(nb*num_gpus)*nb, ldn_local,
                                           h_R+j, lda );
            }
          }
        ...
        /* =====================================================================
          ...
          if (info < 0) {
            printf("Argument %d of cpotrf had an illegal value.\n", -info);
                ...
                printf("lapackf77_cpotrf returned info=%d\n",info );
                break;
          }
