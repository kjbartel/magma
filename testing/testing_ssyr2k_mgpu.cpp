/*
    -- MAGMA (version 1.2.1) --
       ...
       @generated s Thu Jun 28 12:31:51 2012
...
// version from ssytrd_mgpu.cpp
extern "C" void
magma_ssyr2k_mgpu(
    int num_gpus, char uplo, char trans, int nb, int n, int k,
    float alpha, float **db, int lddb, 
    float beta,           float **dc, int lddc, int offset,
...
int main( int argc, char** argv)
{
    ...
    for( int j = 0; j < count; ++j ) {
        ...
        lda  = n;
        ldda = ((n + 31)/32)*32;
        gflops = FLOPS_SSYR2K( (float)k, (float)n ) / 1e9;
        ...
        /* ====================================================================
           ...
           =================================================================== */
        magmablas_ssetmatrix_1D_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb );
        for( int d = 0; d < ngpu; ++d ) {
            cudaSetDevice( d );
            dW[d] = dV[d] + ldda*k;
        ...
        magmablas_sgetmatrix_1D_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb );
        ...
        // Get dA back to the CPU to compare with the CPU result.
        magmablas_sgetmatrix_1D_bcyclic( n, n, dA, ldda, hR2, lda, ngpu, nb );
        ...
        if ( checkres ) {
            // store ||A||
            error = lapackf77_slange("f", &n, &n, hA, &lda, work );
            error2 = error;
            
            //printf( "A=" ); magma_sprint( n, n, hA, lda );
            //printf( "V=" ); magma_sprint( n, k, hV, lda );
            //printf( "W=" ); magma_sprint( n, k, hW, lda );
            
            cpu_time = magma_wtime();
            blasf77_ssyr2k( "Lower", "NoTrans", &n, &k,
                            &c_neg_one, hV, &lda,
                                        hW, &lda,
                            &d_one,     hA, &lda );
            cpu_time = magma_wtime() - cpu_time;
            
            //printf( "Ahat ="   );  magma_sprint( n, n, hA,  lda );
            //printf( "dAhat ="  );  magma_sprint( n, n, hR,  lda );
            //printf( "dAhat2 =" );  magma_sprint( n, n, hR2, lda );
            ...
            error = lapackf77_slansy("fro", "Lower", &n, hR, &lda, work) / error;
            
            blasf77_saxpy( &size, &c_neg_one, hA, &ione, hR2, &ione );
            error2 = lapackf77_slansy("fro", "Lower", &n, hR2, &lda, work) / error2;
