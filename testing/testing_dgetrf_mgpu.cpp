...
int main( int argc, char** argv)
{
    ...
    if (argc != 1){
        ...
        if (M>0 && N>0 && num_gpus0>0)
            printf("  testing_dgetrf_mgpu -M %d -N %d -NGPU %d\n", M, N, num_gpus0);
        else {
            printf("\nError: \n");
    ...
    for(i=0; i<num_gpus; i++){
      ...
      cudaSetDevice(i);
      //TESTING_DEVALLOC( d_lA[i], double, ldda*n_local );
      TESTING_DEVALLOC( d_lA[i], double, ldda*ldn_local );
    }
    cudaSetDevice(0);
    nb0 = nb;
       ...
       /* =====================================================================
        ...
        /* ====================================================================
           ...
           =================================================================== */
                ...
                //cudaSetDevice(0);
        //cublasSetMatrix( M, N, sizeof(double), h_R, lda, d_lA[0], ldda);
        nb = magma_get_dgetrf_nb(M);
#ifdef TESTING_DGETRF_MGPU_CHECK
                if( nb != nb0 ) {
        ...
        if( num_gpus0 > N/nb ) {
                ...
                for(int j=0; j<N; j+=nb){
                  ...
                  cudaSetDevice(k);
                  nk = min(nb, N-j);
                  cublasSetMatrix( M, nk, sizeof(double), h_R+j*lda, lda,
        ...
        start = get_current_time();
        ret = magma_dgetrf_mgpu( num_gpus, M, N, d_lA, ldda, ipiv, &info);
        end = get_current_time();
        ...
        if (ret != MAGMA_SUCCESS) {
            printf("magma_dgetrf_mgpu returned with error code %d\n", ret);
                        break;
                }
                /* == download the matrix from GPUs == */
                //cudaSetDevice(0);
                //cublasGetMatrix( M, N, sizeof(double), d_lA[0], ldda, h_R, M);
        for(int j=0; j<N; j+=nb){
                  k = (j/nb)%num_gpus;
                  cudaSetDevice(k);
                  nk = min(nb, N-j);
