...
int main( int argc, char** argv)
{
    ...
    float *hA, *hX, *hB, *hR;
    float *dA[MagmaMaxGPUs], *dX[MagmaMaxGPUs], *dB[MagmaMaxGPUs];
    float *dA2;
    ...
    TESTING_MALLOC( hR, float, lda*n*2 );
    ...
    for( int j = 0; j < count; ++j ) {
        ...
        lda  = m;
        ldda = ((m + 31)/32)*32;
        gflops = FLOPS_SSYMM( MagmaLeft, (float)m, (float)n ) / 1e9;
        ...
        /* ====================================================================
           ...
           =================================================================== */
        magmablas_ssetmatrix_1D_bcyclic( m, m, hA, lda, dA, ldda, ngpu, nb );
        for( int d = 0; d < ngpu; ++d ) {
            cudaSetDevice( d );
        ...
        if ( checkres ) {
            // store ||A||*||X||
            error  = lapackf77_slange("fro", &m, &m, hA, &lda, work );
            error *= lapackf77_slange("fro", &m, &n, hX, &lda, work );
            ...
            //printf( "B  =" ); magma_sprint( m, n, hB, lda );
            //printf( "dB =" ); magma_sprint( m, n, hR, lda );
            ...
            error = lapackf77_slange("fro", &m, &n, hR, &lda, work) / error;
