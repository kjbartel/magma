       ...
       /* =====================================================================
        ...
        if( num_gpus0 > N/nb ) {
                ...
                for(int j=0; j<N; j+=nb){
                  ...
                  cudaSetDevice(k);
                  nk = min(nb, N-j);
                  cublasSetMatrix( M, nk, sizeof(cuFloatComplex), h_R+j*lda, lda,
        ...
        start = get_current_time();
        ret = magma_cgetrf_mgpu( num_gpus, M, N, d_lA, ldda, ipiv, &info);
        end = get_current_time();
        ...
        if (ret != MAGMA_SUCCESS) {
            printf("magma_cgetrf_mgpu returned with error code %d\n", ret);
                        break;
                }
