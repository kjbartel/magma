...
int main( int argc, char** argv) 
{
    ...
    printf( "npivots=%d\n", npivots );
    printf("        cublasDswap       dswap             dswapblk          dlaswp   dpermute dlaswp2  dlaswpx           dcopymatrix                \n");
    ...
    for( int cnt = 0; cnt < count; ++cnt ) {
        ...
        gbytes = 2. * sizeof(double) * npivots * N / 1e9;
        ...
        time = magma_sync_wtime( queue );
        magmablas_dswapblk( 'R', N, d_A1, ldda, d_A2, ldda, 1, npivots, ipiv, 1, 0);
        time = magma_sync_wtime( queue ) - time;
        ...
        time = magma_sync_wtime( queue );
        magmablas_dswapblk( 'C', N, d_A1, ldda, d_A2, ldda, 1, npivots, ipiv, 1, 0);
        time = magma_sync_wtime( queue ) - time;
        ...
        /* Row Major */
        memcpy( ipiv2, ipiv, npivots*sizeof(magma_int_t) );  // dpermute updates ipiv2
        init_matrix( N, N, h_A1, lda, 0 );
        magma_dsetmatrix( N, N, h_A1, lda, d_A1, ldda );

        time = magma_sync_wtime( queue );
        magmablas_dpermute_long2( N, d_A1, ldda, ipiv2, npivots, 0 );
        time = magma_sync_wtime( queue ) - time;
        ...
        time = magma_sync_wtime( queue );
        magmablas_dlaswp( N, d_A1, ldda, 1, npivots, ipiv, 1);
        time = magma_sync_wtime( queue ) - time;
        ...
        magmablas_dlaswp2( N, d_A1, ldda, 1, npivots, d_ipiv );
        time = magma_sync_wtime( queue ) - time;
        ...
        time = magma_sync_wtime( queue );
        magmablas_dlaswpx( N, d_A1, ldda, 1, 1, npivots, ipiv, 1);
        time = magma_sync_wtime( queue ) - time;
        ...
        time = magma_sync_wtime( queue );
        magmablas_dlaswpx( N, d_A1, 1, ldda, 1, npivots, ipiv, 1);
        time = magma_sync_wtime( queue ) - time;
        col_perf5 = gbytes / time;

        lapackf77_dlaswp( &N, h_A1, &lda, &ione, &npivots, ipiv, &ione);
        magma_dgetmatrix( N, N, d_A1, ldda, h_R1, lda );
        check += diff_matrix( N, N, h_A1, lda, h_R1, lda )*shift;
        shift *= 2;
        ...
        time = magma_sync_wtime( queue );
        magma_dcopymatrix( N, npivots, d_A1, ldda, d_A2, ldda );
        time = magma_sync_wtime( queue ) - time;
        // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
        col_perf6 = 0.5 * gbytes / time;

        time = magma_sync_wtime( queue );
        magma_dcopymatrix( npivots, N, d_A1, ldda, d_A2, ldda );
        time = magma_sync_wtime( queue ) - time;
        // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
