...
// version from dsytrd_mgpu.cpp
extern "C" void
magma_dsyr2k_mgpu(
    int num_gpus, char uplo, char trans, int nb, int n, int k,
    double alpha, double **db, int lddb, 
    double beta,           double **dc, int lddc, int offset,
...
int main( int argc, char** argv)
{
    ...
    for( int j = 0; j < count; ++j ) {
        ...
        lda  = n;
        ldda = ((n + 31)/32)*32;
        gflops = FLOPS_DSYR2K( (double)k, (double)n ) / 1e9;
        ...
        /* ====================================================================
           ...
           =================================================================== */
        magmablas_dsetmatrix_1D_bcyclic( n, n, hA, lda, dA, ldda, ngpu, nb );
        for( int d = 0; d < ngpu; ++d ) {
            cudaSetDevice( d );
            dW[d] = dV[d] + ldda*k;
        ...
        magmablas_dgetmatrix_1D_bcyclic( n, n, dA, ldda, hR, lda, ngpu, nb );
        ...
        // Get dA back to the CPU to compare with the CPU result.
        magmablas_dgetmatrix_1D_bcyclic( n, n, dA, ldda, hR2, lda, ngpu, nb );
        ...
        if ( checkres ) {
            // store ||A||
            error = lapackf77_dlange("f", &n, &n, hA, &lda, work );
            error2 = error;
            
            //printf( "A=" ); magma_dprint( n, n, hA, lda );
            //printf( "V=" ); magma_dprint( n, k, hV, lda );
            //printf( "W=" ); magma_dprint( n, k, hW, lda );
            
            cpu_time = magma_wtime();
            blasf77_dsyr2k( "Lower", "NoTrans", &n, &k,
                            &c_neg_one, hV, &lda,
                                        hW, &lda,
                            &d_one,     hA, &lda );
            cpu_time = magma_wtime() - cpu_time;
            
            //printf( "Ahat ="   );  magma_dprint( n, n, hA,  lda );
            //printf( "dAhat ="  );  magma_dprint( n, n, hR,  lda );
            //printf( "dAhat2 =" );  magma_dprint( n, n, hR2, lda );
            ...
            error = lapackf77_dlansy("fro", "Lower", &n, hR, &lda, work) / error;
            
            blasf77_daxpy( &size, &c_neg_one, hA, &ione, hR2, &ione );
            error2 = lapackf77_dlansy("fro", "Lower", &n, hR2, &lda, work) / error2;
