/*
    ...
    @generated c Wed Nov 14 22:54:23 2012
...
int main( int argc, char** argv) 
{
    ...
    if (argc != 1){
        ...
        if (N>0)
            printf("  testing_cheevd_gpu -N %d [-JV] [-JN]\n\n", (int) N);
        else {
            printf("\nUsage: \n");
            printf("  testing_cheevd_gpu -N %d [-JV] [-JN]\n\n", (int) N);
            exit(1);
    ...
    TESTING_DEVALLOC(  d_R, cuFloatComplex, N*ldda );
    ...
    for(i=0; i<8; i++){
        ...
        for(int k=0;k<N;k++)
        {
            MAGMA_C_SET2REAL(h_A[k*N+k],  MAGMA_C_REAL(h_A[k*N+k]));
        }
        magma_csetmatrix( N, N, h_A, N, d_R, ldda );
        ...
        if ( checkres ) {
          ...
          magma_cgetmatrix( N, N, d_R, ldda, h_R, N );
