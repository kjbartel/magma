...
int main( int argc, char** argv )
{
    ...
    cuFloatComplex *dC, *dA, *dT;
    TESTING_DEVALLOC( dC, cuFloatComplex, ldc*n );
    TESTING_DEVALLOC( dA, cuFloatComplex, lda*k );
    TESTING_DEVALLOC( dT, cuFloatComplex, dt_size );
    ...
    for( int i = 0; i < ntest; ++i ) {
        ...
        for( int itran = 0; itran < 2; ++itran ) {
            ...
            if (info != 0)
                printf("magma_cunmqr_gpu returned error %d (lwork query).\n", (int) info);
            ...
            if (info != 0)
                printf("magma_cunmqr_gpu returned error %d.\n", (int) info);
