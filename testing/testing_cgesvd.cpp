...
int main( int argc, char** argv) 
{
    ...
    for(i=0; i<8; i++){
        ...
        if ( checkres ) {
          ...
          cuFloatComplex *h_work_err;
          magma_int_t lwork_err = max(5*min_mn, (3*min_mn + max(M,N)))*128;
          TESTING_MALLOC(h_work_err, cuFloatComplex, lwork_err);
          
          #if defined(PRECISION_z) || defined(PRECISION_c)
             lapackf77_cbdt01(&M, &N, &izero, h_A, &M,
                              U, &M, S1, E, VT, &N, h_work_err, rwork, &result[0]);
             if (M != 0 && N != 0) {
               lapackf77_cunt01("Columns",&M,&M, U,&M, h_work_err,&lwork_err, rwork, &result[1]);
               lapackf77_cunt01(   "Rows",&N,&N,VT,&N, h_work_err,&lwork_err, rwork, &result[2]);
             }
          #else
             lapackf77_cbdt01(&M, &N, &izero, h_A, &M,
                              U, &M, S1, E, VT, &N, h_work_err, &result[0]);
             if (M != 0 && N != 0) {
               lapackf77_cunt01("Columns",&M,&M, U,&M, h_work_err, &lwork_err, &result[1]);
               lapackf77_cunt01(   "Rows",&N,&N,VT,&N, h_work_err, &lwork_err, &result[2]);
             }
          #endif
