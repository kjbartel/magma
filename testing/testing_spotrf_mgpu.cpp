 ...
 *     November 2011
 *
 * @generated s Sun Nov 13 20:48:49 2011
 *
 **/
/* includes, system */
#include <stdlib.h>
...
int main( int argc, char** argv) 
{
    ...
    if (argc != 1){
          ...
          printf("\n  testing_spotrf_gpu -N %d -NGPU %d -UPLO L\n\n", N, num_gpus0 );
          else
          printf("\n  testing_spotrf_gpu -N %d -NGPU %d -UPLO U\n\n", N, num_gpus0 );
    } else {
          printf("\nDefault: \n");
          printf("  testing_spotrf_gpu -N %d:%d -NGPU %d -UPLO L\n\n", size[0],size[n_sizes-1], num_gpus0 );
    }
        if( N <= 0 || num_gpus0 <= 0 )  {
    ...
    for(i=0; i<n_sizes; i++){
      ...
      if( num_gpus0 > N/nb ) {
          ...
          if( lapackf77_lsame(uplo, "U") ) {
            for(j=0; j<N; j+=nb){
              ...
              cudaSetDevice(k);
              nk = min(nb, N-j);
              cublasSetMatrix( N, nk, sizeof(float), h_A+j*lda, lda,
            ...
            for(j=0; j<N; j+=nb){
              ...
              cudaSetDevice(k);
              nk = min(nb, N-j);
              cublasSetMatrix( nk, N, sizeof(float), h_A+j, lda,
                               d_lA[k]+j/(nb*num_gpus)*nb, ldn_local);
            }
          }
          cudaSetDevice(0);

          /* call magma_spotrf_mgp */
      ...
      gpu_perf = flops / GetTimerValue(start, end);
        ...
        for(j=0; j<N; j+=nb){
              ...
              cudaSetDevice(k);
              nk = min(nb, N-j);
              cublasGetMatrix( N, nk, sizeof(float),
                               d_lA[k]+j/(nb*num_gpus)*nb*ldda, ldda,
                               h_R+j*lda, lda);
                }
          } else {
            for(j=0; j<N; j+=nb){
              ...
              cudaSetDevice(k);
              nk = min(nb, N-j);
              cublasGetMatrix( nk, N, sizeof(float), 
                               d_lA[k]+j/(nb*num_gpus)*nb, ldn_local,
                                           h_R+j, lda );
            }
          }
