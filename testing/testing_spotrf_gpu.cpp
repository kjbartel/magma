...
int main( int argc, char** argv) 
{
    ...
    for(i=0; i<10; i++){
        ...
        float *dwork, maxnorm;
        TESTING_DEVALLOC(  dwork, float, size[9] );

        start = get_current_time();
        magmablas_slascl('U', 1, 1, 1., 2., N, N, d_A, ldda, &info);
        end = get_current_time();
        printf("slascl GB/s = %f, time (ms) = %f\n",
               16.*N*N/(1000000.*GetTimerValue(start, end)),
               GetTimerValue(start, end));

        start = get_current_time();
        maxnorm = magmablas_slansy('M', 'L',N, d_A, ldda, dwork);
        end = get_current_time();
