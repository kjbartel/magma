...
int main( int argc, char** argv)
{
    ...
    cuFloatComplex *h_A, *h_R, *d_R, *h_Q, *h_work, *work;
    cuFloatComplex *tau;
    ...
    for(i=0; i<10; i++){
        ...
        /* Make the matrix hermitian */
        {
            magma_int_t i, j;
            for(i=0; i<N; i++) {
                MAGMA_C_SET2REAL( h_A[i*lda+i], ( MAGMA_C_GET_X(h_A[i*lda+i]) ) );
                for(j=0; j<i; j++)
                    h_A[i*lda+j] = cuConjf(h_A[j*lda+i]);
            }
        }
        cublasSetMatrix(N, N, sizeof(cuFloatComplex), h_A, lda, d_R, lda);
        ...
        if ( checkres ) {
            ...
            cublasGetMatrix(N, N, sizeof(cuFloatComplex), d_R, lda, h_R, lda);
            cublasGetMatrix(N, N, sizeof(cuFloatComplex), d_R, lda, h_Q, lda);
