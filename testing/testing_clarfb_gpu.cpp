 ...
 *
 * @author Mark Gates
 * @generated c Tue May 15 18:18:23 2012
 *
 **/
...
int main( int argc, char** argv )
{
    ...
    for( int it = 0; it < 2; ++it ) {
        ...
        // C is full
        size = ldc*n;
        lapackf77_clarnv( &idist, iseed, &size, C );
        ...
        nv   = k;
        size = ldv*nv;
        lapackf77_clarnv( &idist, iseed, &size, V );
        if ( *storev[iv] == 'C' ) {
            if ( *direct[id] == 'F' ) {
                lapackf77_claset( "Upper", &k, &k, &c_zero, &c_one, V, &ldv );
            }
            else {
                lapackf77_claset( "Lower", &k, &k, &c_zero, &c_one, &V[(ldv-k)], &ldv );
            }
        }
        else {
            ...
            std::swap( ldv, nv );
            if ( *direct[id] == 'F' ) {
                lapackf77_claset( "Lower", &k, &k, &c_zero, &c_one, V, &ldv );
            }
            else {
                lapackf77_claset( "Upper", &k, &k, &c_zero, &c_one, &V[(nv-k)*ldv], &ldv );
            }
        }
        //printf( "# ldv %d, nv %d\n", ldv, nv );
        ...
        magma_int_t k1 = k-1;
        size = ldt*k;
        lapackf77_clarnv( &idist, iseed, &size, T );
        if ( *direct[id] == 'F' ) {
            lapackf77_claset( "Lower", &k1, &k1, &c_zero, &c_zero, &T[1], &ldt );
        }
        else {
            lapackf77_claset( "Upper", &k1, &k1, &c_zero, &c_zero, &T[1*ldt], &ldt );
        }
        ...
        lapackf77_clarfb( side[is], trans[it], direct[id], storev[iv],
                          &m, &n, &k,
                          V, &ldv, T, &ldt, C, &ldc, W, &ldw );
        //printf( "HC=" );  magma_cprint( m, n, C, ldc );
        
        magma_clarfb_gpu( *side[is], *trans[it], *direct[id], *storev[iv],
                          m, n, k,
