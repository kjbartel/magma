...
int main( int argc, char** argv) 
{
    ...
    for(i=0; i<10; i++){
        ...
        if (info < 0)
            printf("Argument %d of lapack_sgeqp3 had an illegal value.\n", -info);
        ...
        if (info < 0)
            printf("Argument %d of magma_sgeqp3 had an illegal value.\n", -info);
        ...
        if ( checkres ) 
            {
                ...
                result[0] = lapackf77_sqpt01(&M, &N, &minmn, h_A, h_R, &lda, tau, jpvt, h_work, &lwork);
                result[0] *= ulp;
