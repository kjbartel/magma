/*
    -- MAGMA (version 1.2.1) --
       ...
       @generated d Thu Jun 28 12:31:42 2012
...
int main( int argc, char** argv )
{
    ...
    TESTING_HOSTALLOC( hwork, double, lwork     );
    ...
    for( int i = 0; i < ntest; ++i ){
        ...
        if ( info != 0 )
            printf("magma_dgeqrf_gpu return error %d\n", info );
        ...
        if ( info != 0 )
            printf("magma_dorgqr_gpu return error %d\n", info );
        ...
        if ( checkres ) {
            ...
            lapackf77_dgeqrf( &m, &n, hA, &lda, tau, hwork, &lwork, &info );
            if ( info != 0 )
                printf("lapackf77_dgeqrf return error %d\n", info );
            
            cpu_time = magma_wtime();
            lapackf77_dorgqr( &m, &n, &k, hA, &lda, tau, hwork, &lwork, &info );
            cpu_time = magma_wtime() - cpu_time;
            if ( info != 0 )
                printf("lapackf77_dorgqr return error %d\n", info );
