 ...
 *
 * @author Mark Gates
 * @generated s Tue May 15 18:18:27 2012
 *
 **/
#include <stdio.h>
...
int main( int argc, char** argv )
{
    ...
    assert( piv != NULL );
    err = magma_smalloc_host( &A , size );  assert( err == 0 );
    err = magma_smalloc_host( &B , size );  assert( err == 0 );
    err = magma_smalloc_host( &C , size );  assert( err == 0 );
    err = magma_smalloc_host( &C2, size );  assert( err == 0 );
